/* Generated By:JavaCC: Do not edit this line. ParserTokenManager.java */
package org.wcs.smart.query.parser.internal.parser;
import java.util.*;
import org.wcs.smart.query.parser.internal.filter.*;
import org.wcs.smart.query.parser.internal.summary.*;

/** Token Manager. */
public class ParserTokenManager implements ParserConstants
{

  /** Debug output. */
  public  java.io.PrintStream debugStream = System.out;
  /** Set debug output. */
  public  void setDebugStream(java.io.PrintStream ds) { debugStream = ds; }
private int jjStopAtPos(int pos, int kind)
{
   jjmatchedKind = kind;
   jjmatchedPos = pos;
   return pos + 1;
}
private int jjMoveStringLiteralDfa0_0()
{
   switch(curChar)
   {
      case 9:
         jjmatchedKind = 2;
         return jjMoveNfa_0(3, 0);
      case 10:
         jjmatchedKind = 4;
         return jjMoveNfa_0(3, 0);
      case 13:
         jjmatchedKind = 3;
         return jjMoveNfa_0(3, 0);
      case 32:
         jjmatchedKind = 1;
         return jjMoveNfa_0(3, 0);
      case 33:
         return jjMoveStringLiteralDfa1_0(0x800000L, 0x0L);
      case 37:
         jjmatchedKind = 31;
         return jjMoveNfa_0(3, 0);
      case 40:
         jjmatchedKind = 25;
         return jjMoveNfa_0(3, 0);
      case 41:
         jjmatchedKind = 26;
         return jjMoveNfa_0(3, 0);
      case 42:
         jjmatchedKind = 29;
         return jjMoveNfa_0(3, 0);
      case 43:
         jjmatchedKind = 32;
         return jjMoveNfa_0(3, 0);
      case 44:
         jjmatchedKind = 17;
         return jjMoveNfa_0(3, 0);
      case 45:
         jjmatchedKind = 33;
         return jjMoveNfa_0(3, 0);
      case 47:
         jjmatchedKind = 30;
         return jjMoveNfa_0(3, 0);
      case 60:
         jjmatchedKind = 18;
         return jjMoveStringLiteralDfa1_0(0x1080000L, 0x0L);
      case 61:
         jjmatchedKind = 22;
         return jjMoveNfa_0(3, 0);
      case 62:
         jjmatchedKind = 20;
         return jjMoveStringLiteralDfa1_0(0x200000L, 0x0L);
      case 65:
         return jjMoveStringLiteralDfa1_0(0x80L, 0x0L);
      case 67:
         return jjMoveStringLiteralDfa1_0(0x100L, 0x0L);
      case 69:
         return jjMoveStringLiteralDfa1_0(0xe00L, 0x0L);
      case 73:
         return jjMoveStringLiteralDfa1_0(0x1000L, 0x0L);
      case 78:
         return jjMoveStringLiteralDfa1_0(0x2000L, 0x0L);
      case 79:
         return jjMoveStringLiteralDfa1_0(0x4000L, 0x0L);
      case 84:
         return jjMoveStringLiteralDfa1_0(0x8000L, 0x0L);
      case 87:
         return jjMoveStringLiteralDfa1_0(0x10000L, 0x0L);
      case 97:
         return jjMoveStringLiteralDfa1_0(0x80L, 0x0L);
      case 98:
         return jjMoveStringLiteralDfa1_0(0x2000000000L, 0x0L);
      case 99:
         return jjMoveStringLiteralDfa1_0(0x400000100L, 0x0L);
      case 101:
         return jjMoveStringLiteralDfa1_0(0x1000000e00L, 0x0L);
      case 105:
         return jjMoveStringLiteralDfa1_0(0x1000L, 0x0L);
      case 110:
         return jjMoveStringLiteralDfa1_0(0x800002000L, 0x0L);
      case 111:
         return jjMoveStringLiteralDfa1_0(0x4000L, 0x0L);
      case 112:
         return jjMoveStringLiteralDfa1_0(0xfc00000000000000L, 0x7L);
      case 116:
         return jjMoveStringLiteralDfa1_0(0x8000L, 0x0L);
      case 119:
         return jjMoveStringLiteralDfa1_0(0x10000L, 0x0L);
      case 123:
         jjmatchedKind = 27;
         return jjMoveNfa_0(3, 0);
      case 124:
         jjmatchedKind = 75;
         return jjMoveNfa_0(3, 0);
      case 125:
         jjmatchedKind = 28;
         return jjMoveNfa_0(3, 0);
      default :
         return jjMoveNfa_0(3, 0);
   }
}
private int jjMoveStringLiteralDfa1_0(long active0, long active1)
{
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 0);
   }
   switch(curChar)
   {
      case 61:
         if ((active0 & 0x80000L) != 0L)
         {
            jjmatchedKind = 19;
            jjmatchedPos = 1;
         }
         else if ((active0 & 0x200000L) != 0L)
         {
            jjmatchedKind = 21;
            jjmatchedPos = 1;
         }
         else if ((active0 & 0x800000L) != 0L)
         {
            jjmatchedKind = 23;
            jjmatchedPos = 1;
         }
         break;
      case 62:
         if ((active0 & 0x1000000L) != 0L)
         {
            jjmatchedKind = 24;
            jjmatchedPos = 1;
         }
         break;
      case 65:
         return jjMoveStringLiteralDfa2_0(active0, 0x100L, active1, 0L);
      case 72:
         return jjMoveStringLiteralDfa2_0(active0, 0x18000L, active1, 0L);
      case 76:
         return jjMoveStringLiteralDfa2_0(active0, 0x200L, active1, 0L);
      case 78:
         if ((active0 & 0x1000L) != 0L)
         {
            jjmatchedKind = 12;
            jjmatchedPos = 1;
         }
         return jjMoveStringLiteralDfa2_0(active0, 0x480L, active1, 0L);
      case 79:
         return jjMoveStringLiteralDfa2_0(active0, 0x2000L, active1, 0L);
      case 82:
         if ((active0 & 0x4000L) != 0L)
         {
            jjmatchedKind = 14;
            jjmatchedPos = 1;
         }
         break;
      case 88:
         return jjMoveStringLiteralDfa2_0(active0, 0x800L, active1, 0L);
      case 97:
         return jjMoveStringLiteralDfa2_0(active0, 0xfc00000000000100L, active1, 0x7L);
      case 101:
         return jjMoveStringLiteralDfa2_0(active0, 0x2000000000L, active1, 0L);
      case 104:
         return jjMoveStringLiteralDfa2_0(active0, 0x18000L, active1, 0L);
      case 108:
         return jjMoveStringLiteralDfa2_0(active0, 0x200L, active1, 0L);
      case 110:
         if ((active0 & 0x1000L) != 0L)
         {
            jjmatchedKind = 12;
            jjmatchedPos = 1;
         }
         return jjMoveStringLiteralDfa2_0(active0, 0x480L, active1, 0L);
      case 111:
         return jjMoveStringLiteralDfa2_0(active0, 0xc00002000L, active1, 0L);
      case 113:
         return jjMoveStringLiteralDfa2_0(active0, 0x1000000000L, active1, 0L);
      case 114:
         if ((active0 & 0x4000L) != 0L)
         {
            jjmatchedKind = 14;
            jjmatchedPos = 1;
         }
         break;
      case 120:
         return jjMoveStringLiteralDfa2_0(active0, 0x800L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 1);
}
private int jjMoveStringLiteralDfa2_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 1);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 1);
   }
   switch(curChar)
   {
      case 68:
         if ((active0 & 0x80L) != 0L)
         {
            jjmatchedKind = 7;
            jjmatchedPos = 2;
         }
         else if ((active0 & 0x400L) != 0L)
         {
            jjmatchedKind = 10;
            jjmatchedPos = 2;
         }
         break;
      case 69:
         return jjMoveStringLiteralDfa3_0(active0, 0x18000L, active1, 0L);
      case 73:
         return jjMoveStringLiteralDfa3_0(active0, 0x800L, active1, 0L);
      case 83:
         return jjMoveStringLiteralDfa3_0(active0, 0x300L, active1, 0L);
      case 84:
         if ((active0 & 0x2000L) != 0L)
         {
            jjmatchedKind = 13;
            jjmatchedPos = 2;
         }
         break;
      case 100:
         if ((active0 & 0x80L) != 0L)
         {
            jjmatchedKind = 7;
            jjmatchedPos = 2;
         }
         else if ((active0 & 0x400L) != 0L)
         {
            jjmatchedKind = 10;
            jjmatchedPos = 2;
         }
         break;
      case 101:
         return jjMoveStringLiteralDfa3_0(active0, 0x18000L, active1, 0L);
      case 105:
         return jjMoveStringLiteralDfa3_0(active0, 0x800L, active1, 0L);
      case 110:
         return jjMoveStringLiteralDfa3_0(active0, 0x400000000L, active1, 0L);
      case 115:
         return jjMoveStringLiteralDfa3_0(active0, 0x300L, active1, 0L);
      case 116:
         if ((active0 & 0x2000L) != 0L)
         {
            jjmatchedKind = 13;
            jjmatchedPos = 2;
         }
         return jjMoveStringLiteralDfa3_0(active0, 0xfc00002800000000L, active1, 0x7L);
      case 117:
         return jjMoveStringLiteralDfa3_0(active0, 0x1000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 2);
}
private int jjMoveStringLiteralDfa3_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 2);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 2);
   }
   switch(curChar)
   {
      case 69:
         if ((active0 & 0x100L) != 0L)
         {
            jjmatchedKind = 8;
            jjmatchedPos = 3;
         }
         else if ((active0 & 0x200L) != 0L)
         {
            jjmatchedKind = 9;
            jjmatchedPos = 3;
         }
         break;
      case 78:
         if ((active0 & 0x8000L) != 0L)
         {
            jjmatchedKind = 15;
            jjmatchedPos = 3;
         }
         else if ((active0 & 0x10000L) != 0L)
         {
            jjmatchedKind = 16;
            jjmatchedPos = 3;
         }
         break;
      case 83:
         return jjMoveStringLiteralDfa4_0(active0, 0x800L, active1, 0L);
      case 97:
         return jjMoveStringLiteralDfa4_0(active0, 0x1000000000L, active1, 0L);
      case 99:
         return jjMoveStringLiteralDfa4_0(active0, 0x800000000L, active1, 0L);
      case 101:
         if ((active0 & 0x100L) != 0L)
         {
            jjmatchedKind = 8;
            jjmatchedPos = 3;
         }
         else if ((active0 & 0x200L) != 0L)
         {
            jjmatchedKind = 9;
            jjmatchedPos = 3;
         }
         break;
      case 110:
         if ((active0 & 0x8000L) != 0L)
         {
            jjmatchedKind = 15;
            jjmatchedPos = 3;
         }
         else if ((active0 & 0x10000L) != 0L)
         {
            jjmatchedKind = 16;
            jjmatchedPos = 3;
         }
         break;
      case 114:
         return jjMoveStringLiteralDfa4_0(active0, 0xfc00000000000000L, active1, 0x7L);
      case 115:
         return jjMoveStringLiteralDfa4_0(active0, 0x800L, active1, 0L);
      case 116:
         return jjMoveStringLiteralDfa4_0(active0, 0x400000000L, active1, 0L);
      case 119:
         return jjMoveStringLiteralDfa4_0(active0, 0x2000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 3);
}
private int jjMoveStringLiteralDfa4_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 3);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 3);
   }
   switch(curChar)
   {
      case 84:
         return jjMoveStringLiteralDfa5_0(active0, 0x800L, active1, 0L);
      case 97:
         return jjMoveStringLiteralDfa5_0(active0, 0x400000000L, active1, 0L);
      case 101:
         return jjMoveStringLiteralDfa5_0(active0, 0x2000000000L, active1, 0L);
      case 108:
         return jjMoveStringLiteralDfa5_0(active0, 0x1000000000L, active1, 0L);
      case 111:
         return jjMoveStringLiteralDfa5_0(active0, 0xfc00000800000000L, active1, 0x7L);
      case 116:
         return jjMoveStringLiteralDfa5_0(active0, 0x800L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 4);
}
private int jjMoveStringLiteralDfa5_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 4);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 4);
   }
   switch(curChar)
   {
      case 83:
         if ((active0 & 0x800L) != 0L)
         {
            jjmatchedKind = 11;
            jjmatchedPos = 5;
         }
         break;
      case 101:
         return jjMoveStringLiteralDfa6_0(active0, 0x2000000000L, active1, 0L);
      case 105:
         return jjMoveStringLiteralDfa6_0(active0, 0x400000000L, active1, 0L);
      case 108:
         return jjMoveStringLiteralDfa6_0(active0, 0xfc00000000000000L, active1, 0x7L);
      case 110:
         return jjMoveStringLiteralDfa6_0(active0, 0x800000000L, active1, 0L);
      case 115:
         if ((active0 & 0x800L) != 0L)
         {
            jjmatchedKind = 11;
            jjmatchedPos = 5;
         }
         else if ((active0 & 0x1000000000L) != 0L)
         {
            jjmatchedKind = 36;
            jjmatchedPos = 5;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 5);
}
private int jjMoveStringLiteralDfa6_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 5);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 5);
   }
   switch(curChar)
   {
      case 58:
         return jjMoveStringLiteralDfa7_0(active0, 0xfc00000000000000L, active1, 0x7L);
      case 110:
         if ((active0 & 0x2000000000L) != 0L)
         {
            jjmatchedKind = 37;
            jjmatchedPos = 6;
         }
         return jjMoveStringLiteralDfa7_0(active0, 0x400000000L, active1, 0L);
      case 116:
         return jjMoveStringLiteralDfa7_0(active0, 0x800000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 6);
}
private int jjMoveStringLiteralDfa7_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 6);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 6);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa8_0(active0, 0x800000000L, active1, 0L);
      case 105:
         return jjMoveStringLiteralDfa8_0(active0, 0x400000000000000L, active1, 0L);
      case 108:
         return jjMoveStringLiteralDfa8_0(active0, 0L, active1, 0x1L);
      case 109:
         return jjMoveStringLiteralDfa8_0(active0, 0x2000000000000000L, active1, 0x4L);
      case 112:
         return jjMoveStringLiteralDfa8_0(active0, 0x4000000000000000L, active1, 0x2L);
      case 115:
         if ((active0 & 0x400000000L) != 0L)
         {
            jjmatchedKind = 34;
            jjmatchedPos = 7;
         }
         return jjMoveStringLiteralDfa8_0(active0, 0x800000000000000L, active1, 0L);
      case 116:
         return jjMoveStringLiteralDfa8_0(active0, 0x9000000000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 7);
}
private int jjMoveStringLiteralDfa8_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 7);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 7);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa9_0(active0, 0x6000000000000000L, active1, 0L);
      case 100:
         if ((active0 & 0x400000000000000L) != 0L)
         {
            jjmatchedKind = 58;
            jjmatchedPos = 8;
         }
         break;
      case 101:
         return jjMoveStringLiteralDfa9_0(active0, 0x1000000000000000L, active1, 0x5L);
      case 105:
         return jjMoveStringLiteralDfa9_0(active0, 0x800000000L, active1, 0x2L);
      case 114:
         return jjMoveStringLiteralDfa9_0(active0, 0x8000000000000000L, active1, 0L);
      case 116:
         return jjMoveStringLiteralDfa9_0(active0, 0x800000000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 8);
}
private int jjMoveStringLiteralDfa9_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 8);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 8);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa10_0(active0, 0x9800000000000000L, active1, 0x1L);
      case 108:
         return jjMoveStringLiteralDfa10_0(active0, 0L, active1, 0x2L);
      case 109:
         return jjMoveStringLiteralDfa10_0(active0, 0L, active1, 0x4L);
      case 110:
         return jjMoveStringLiteralDfa10_0(active0, 0x2000000800000000L, active1, 0L);
      case 116:
         return jjMoveStringLiteralDfa10_0(active0, 0x4000000000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 9);
}
private int jjMoveStringLiteralDfa10_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 9);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 9);
   }
   switch(curChar)
   {
      case 98:
         return jjMoveStringLiteralDfa11_0(active0, 0L, active1, 0x4L);
      case 100:
         return jjMoveStringLiteralDfa11_0(active0, 0x2000000000000000L, active1, 0x1L);
      case 109:
         if ((active0 & 0x1000000000000000L) != 0L)
         {
            jjmatchedKind = 60;
            jjmatchedPos = 10;
         }
         break;
      case 110:
         return jjMoveStringLiteralDfa11_0(active0, 0x8000000000000000L, active1, 0L);
      case 111:
         return jjMoveStringLiteralDfa11_0(active0, 0L, active1, 0x2L);
      case 114:
         return jjMoveStringLiteralDfa11_0(active0, 0x4000000000000000L, active1, 0L);
      case 115:
         if ((active0 & 0x800000000L) != 0L)
         {
            jjmatchedKind = 35;
            jjmatchedPos = 10;
         }
         break;
      case 116:
         return jjMoveStringLiteralDfa11_0(active0, 0x800000000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 10);
}
private int jjMoveStringLiteralDfa11_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 10);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 10);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa12_0(active0, 0x2000000000000000L, active1, 0L);
      case 101:
         return jjMoveStringLiteralDfa12_0(active0, 0L, active1, 0x5L);
      case 105:
         return jjMoveStringLiteralDfa12_0(active0, 0x800000000000000L, active1, 0L);
      case 111:
         return jjMoveStringLiteralDfa12_0(active0, 0x4000000000000000L, active1, 0L);
      case 115:
         return jjMoveStringLiteralDfa12_0(active0, 0x8000000000000000L, active1, 0L);
      case 116:
         if ((active1 & 0x2L) != 0L)
         {
            jjmatchedKind = 65;
            jjmatchedPos = 11;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 11);
}
private int jjMoveStringLiteralDfa12_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 11);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 11);
   }
   switch(curChar)
   {
      case 108:
         return jjMoveStringLiteralDfa13_0(active0, 0x4000000000000000L, active1, 0L);
      case 111:
         return jjMoveStringLiteralDfa13_0(active0, 0x800000000000000L, active1, 0L);
      case 112:
         return jjMoveStringLiteralDfa13_0(active0, 0x8000000000000000L, active1, 0L);
      case 114:
         if ((active1 & 0x1L) != 0L)
         {
            jjmatchedKind = 64;
            jjmatchedPos = 12;
         }
         else if ((active1 & 0x4L) != 0L)
         {
            jjmatchedKind = 66;
            jjmatchedPos = 12;
         }
         break;
      case 116:
         return jjMoveStringLiteralDfa13_0(active0, 0x2000000000000000L, active1, 0L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 12);
}
private int jjMoveStringLiteralDfa13_0(long old0, long active0, long old1, long active1)
{
   if (((active0 &= old0) | (active1 &= old1)) == 0L)
      return jjMoveNfa_0(3, 12);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 12);
   }
   switch(curChar)
   {
      case 101:
         if ((active0 & 0x2000000000000000L) != 0L)
         {
            jjmatchedKind = 61;
            jjmatchedPos = 13;
         }
         break;
      case 110:
         if ((active0 & 0x800000000000000L) != 0L)
         {
            jjmatchedKind = 59;
            jjmatchedPos = 13;
         }
         break;
      case 111:
         return jjMoveStringLiteralDfa14_0(active0, 0x8000000000000000L);
      case 116:
         return jjMoveStringLiteralDfa14_0(active0, 0x4000000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 13);
}
private int jjMoveStringLiteralDfa14_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 13);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 13);
   }
   switch(curChar)
   {
      case 114:
         return jjMoveStringLiteralDfa15_0(active0, 0x8000000000000000L);
      case 121:
         return jjMoveStringLiteralDfa15_0(active0, 0x4000000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 14);
}
private int jjMoveStringLiteralDfa15_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 14);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 14);
   }
   switch(curChar)
   {
      case 112:
         return jjMoveStringLiteralDfa16_0(active0, 0x4000000000000000L);
      case 116:
         if ((active0 & 0x8000000000000000L) != 0L)
         {
            jjmatchedKind = 63;
            jjmatchedPos = 15;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 15);
}
private int jjMoveStringLiteralDfa16_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 15);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 15);
   }
   switch(curChar)
   {
      case 101:
         if ((active0 & 0x4000000000000000L) != 0L)
         {
            jjmatchedKind = 62;
            jjmatchedPos = 16;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 16);
}
static final long[] jjbitVec0 = {
   0x0L, 0x0L, 0xffffffffffffffffL, 0xffffffffffffffffL
};
private int jjMoveNfa_0(int startState, int curPos)
{
   int strKind = jjmatchedKind;
   int strPos = jjmatchedPos;
   int seenUpto;
   input_stream.backup(seenUpto = curPos + 1);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) { throw new Error("Internal Error"); }
   curPos = 0;
   int startsAt = 0;
   jjnewStateCnt = 838;
   int i = 1;
   jjstateSet[0] = startState;
   int kind = 0x7fffffff;
   for (;;)
   {
      if (++jjround == 0x7fffffff)
         ReInitRounds();
      if (curChar < 64)
      {
         long l = 1L << curChar;
         do
         {
            switch(jjstateSet[--i])
            {
               case 3:
                  if ((0x3ff000000000000L & l) != 0L)
                  {
                     if (kind > 38)
                        kind = 38;
                     jjCheckNAddStates(0, 6);
                  }
                  else if ((0x280000000000L & l) != 0L)
                     jjAddStates(7, 10);
                  else if (curChar == 47)
                     jjAddStates(11, 12);
                  else if (curChar == 46)
                     jjCheckNAddTwoStates(20, 22);
                  else if (curChar == 34)
                     jjCheckNAddStates(13, 15);
                  break;
               case 9:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjstateSet[jjnewStateCnt++] = 9;
                  break;
               case 13:
                  if (curChar == 34)
                     jjCheckNAddStates(13, 15);
                  break;
               case 14:
                  if ((0xfffffffbffffdbffL & l) != 0L)
                     jjCheckNAddStates(13, 15);
                  break;
               case 16:
                  if ((0xffffffffffffdbffL & l) != 0L)
                     jjCheckNAddStates(13, 15);
                  break;
               case 17:
                  if (curChar == 34 && kind > 81)
                     kind = 81;
                  break;
               case 18:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(7, 10);
                  break;
               case 19:
                  if (curChar == 46)
                     jjCheckNAdd(20);
                  break;
               case 20:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(20);
                  break;
               case 21:
                  if (curChar == 46)
                     jjCheckNAdd(22);
                  break;
               case 22:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(22, 23);
                  break;
               case 24:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(16, 17);
                  break;
               case 25:
                  if (curChar == 46)
                     jjCheckNAdd(26);
                  break;
               case 26:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(26);
                  break;
               case 27:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddStates(18, 20);
                  break;
               case 28:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(28);
                  break;
               case 29:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(29, 30);
                  break;
               case 30:
                  if (curChar == 46)
                     jjCheckNAdd(31);
                  break;
               case 31:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(31);
                  break;
               case 32:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddStates(21, 24);
                  break;
               case 33:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(33, 23);
                  break;
               case 34:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddStates(25, 27);
                  break;
               case 35:
                  if (curChar == 46)
                     jjCheckNAdd(36);
                  break;
               case 36:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(36, 23);
                  break;
               case 37:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddStates(28, 30);
                  break;
               case 38:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(38);
                  break;
               case 39:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(39, 40);
                  break;
               case 40:
                  if (curChar == 46)
                     jjCheckNAdd(41);
                  break;
               case 41:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(41);
                  break;
               case 42:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(20, 22);
                  break;
               case 43:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddStates(0, 6);
                  break;
               case 44:
                  if (curChar == 47)
                     jjAddStates(11, 12);
                  break;
               case 45:
                  if (curChar != 47)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjCheckNAdd(46);
                  break;
               case 46:
                  if ((0xffffffffffffdbffL & l) == 0L)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjCheckNAdd(46);
                  break;
               case 47:
                  if (curChar == 42)
                     jjCheckNAddTwoStates(48, 49);
                  break;
               case 48:
                  if ((0xfffffbffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(48, 49);
                  break;
               case 49:
                  if (curChar == 42)
                     jjCheckNAddStates(31, 33);
                  break;
               case 50:
                  if ((0xffff7bffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(51, 49);
                  break;
               case 51:
                  if ((0xfffffbffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(51, 49);
                  break;
               case 52:
                  if (curChar == 47 && kind > 6)
                     kind = 6;
                  break;
               case 93:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 94;
                  break;
               case 95:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjstateSet[jjnewStateCnt++] = 95;
                  break;
               case 103:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 104;
                  break;
               case 105:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(34, 35);
                  break;
               case 106:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 107;
                  break;
               case 108:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 49)
                     kind = 49;
                  jjstateSet[jjnewStateCnt++] = 108;
                  break;
               case 110:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 109;
                  break;
               case 120:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 119;
                  break;
               case 128:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 129;
                  break;
               case 130:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(36, 37);
                  break;
               case 131:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 132;
                  break;
               case 133:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjstateSet[jjnewStateCnt++] = 133;
                  break;
               case 135:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 134;
                  break;
               case 145:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 144;
                  break;
               case 153:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 154;
                  break;
               case 155:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(38, 39);
                  break;
               case 156:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 157;
                  break;
               case 158:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 51)
                     kind = 51;
                  jjstateSet[jjnewStateCnt++] = 158;
                  break;
               case 160:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 159;
                  break;
               case 170:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 169;
                  break;
               case 178:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 179;
                  break;
               case 180:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(40, 41);
                  break;
               case 181:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 182;
                  break;
               case 183:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 52)
                     kind = 52;
                  jjstateSet[jjnewStateCnt++] = 183;
                  break;
               case 185:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 184;
                  break;
               case 195:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 194;
                  break;
               case 203:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 204;
                  break;
               case 205:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(42, 43);
                  break;
               case 206:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 207;
                  break;
               case 208:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 53)
                     kind = 53;
                  jjstateSet[jjnewStateCnt++] = 208;
                  break;
               case 210:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 209;
                  break;
               case 220:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 219;
                  break;
               case 228:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 229;
                  break;
               case 230:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 70)
                     kind = 70;
                  jjstateSet[jjnewStateCnt++] = 230;
                  break;
               case 234:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 233;
                  break;
               case 242:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 243;
                  break;
               case 244:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(44, 45);
                  break;
               case 245:
                  if (curChar == 58)
                     jjAddStates(46, 48);
                  break;
               case 247:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 248;
                  break;
               case 249:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 72)
                     kind = 72;
                  jjstateSet[jjnewStateCnt++] = 249;
                  break;
               case 261:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 260;
                  break;
               case 271:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 270;
                  break;
               case 279:
                  if (curChar == 58)
                     jjAddStates(49, 51);
                  break;
               case 280:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(52, 55);
                  break;
               case 281:
                  if (curChar == 46)
                     jjCheckNAdd(282);
                  break;
               case 282:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(282, 283);
                  break;
               case 283:
                  if (curChar != 58)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddTwoStates(284, 286);
                  break;
               case 285:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddTwoStates(285, 286);
                  break;
               case 286:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 287;
                  break;
               case 288:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddTwoStates(286, 288);
                  break;
               case 289:
                  if (curChar == 46)
                     jjCheckNAdd(290);
                  break;
               case 290:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(56, 58);
                  break;
               case 292:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(59, 60);
                  break;
               case 293:
                  if (curChar == 46)
                     jjCheckNAdd(294);
                  break;
               case 294:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(294, 283);
                  break;
               case 295:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(61, 64);
                  break;
               case 296:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(296, 283);
                  break;
               case 297:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(65, 67);
                  break;
               case 298:
                  if (curChar == 46)
                     jjCheckNAdd(299);
                  break;
               case 299:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(299, 283);
                  break;
               case 300:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(68, 72);
                  break;
               case 301:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(73, 75);
                  break;
               case 302:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(76, 79);
                  break;
               case 303:
                  if (curChar == 46)
                     jjCheckNAdd(304);
                  break;
               case 304:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(80, 82);
                  break;
               case 305:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(83, 86);
                  break;
               case 306:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(306, 283);
                  break;
               case 307:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(87, 89);
                  break;
               case 308:
                  if (curChar == 46)
                     jjCheckNAdd(309);
                  break;
               case 309:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(309, 283);
                  break;
               case 310:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(282, 290);
                  break;
               case 311:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(90, 97);
                  break;
               case 319:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 320;
                  break;
               case 321:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(98, 99);
                  break;
               case 322:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 342;
                  break;
               case 323:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 324;
                  break;
               case 325:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(100, 101);
                  break;
               case 326:
                  if (curChar != 58)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(327, 329);
                  break;
               case 328:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(328, 329);
                  break;
               case 329:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 330;
                  break;
               case 331:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(329, 331);
                  break;
               case 333:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 332;
                  break;
               case 350:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 351;
                  break;
               case 352:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(102, 103);
                  break;
               case 353:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 400;
                  break;
               case 354:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 355;
                  break;
               case 356:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(104, 105);
                  break;
               case 357:
                  if (curChar == 58)
                     jjAddStates(106, 108);
                  break;
               case 358:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(109, 112);
                  break;
               case 359:
                  if (curChar == 46)
                     jjCheckNAdd(360);
                  break;
               case 360:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(360, 361);
                  break;
               case 361:
                  if (curChar != 58)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(362, 364);
                  break;
               case 363:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(363, 364);
                  break;
               case 364:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 365;
                  break;
               case 366:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(364, 366);
                  break;
               case 367:
                  if (curChar == 46)
                     jjCheckNAdd(368);
                  break;
               case 368:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(113, 115);
                  break;
               case 370:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(116, 117);
                  break;
               case 371:
                  if (curChar == 46)
                     jjCheckNAdd(372);
                  break;
               case 372:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(372, 361);
                  break;
               case 373:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(118, 121);
                  break;
               case 374:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(374, 361);
                  break;
               case 375:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(122, 124);
                  break;
               case 376:
                  if (curChar == 46)
                     jjCheckNAdd(377);
                  break;
               case 377:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(377, 361);
                  break;
               case 378:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(125, 129);
                  break;
               case 379:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(130, 132);
                  break;
               case 380:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(133, 136);
                  break;
               case 381:
                  if (curChar == 46)
                     jjCheckNAdd(382);
                  break;
               case 382:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(137, 139);
                  break;
               case 383:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(140, 143);
                  break;
               case 384:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(384, 361);
                  break;
               case 385:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(144, 146);
                  break;
               case 386:
                  if (curChar == 46)
                     jjCheckNAdd(387);
                  break;
               case 387:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(387, 361);
                  break;
               case 388:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(360, 368);
                  break;
               case 389:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(147, 154);
                  break;
               case 391:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 390;
                  break;
               case 409:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 410;
                  break;
               case 411:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjstateSet[jjnewStateCnt++] = 411;
                  break;
               case 413:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 412;
                  break;
               case 422:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 423;
                  break;
               case 424:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjstateSet[jjnewStateCnt++] = 424;
                  break;
               case 426:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 425;
                  break;
               case 435:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 436;
                  break;
               case 437:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 46)
                     kind = 46;
                  jjstateSet[jjnewStateCnt++] = 437;
                  break;
               case 439:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 438;
                  break;
               case 448:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 449;
                  break;
               case 450:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 47)
                     kind = 47;
                  jjstateSet[jjnewStateCnt++] = 450;
                  break;
               case 452:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 451;
                  break;
               case 461:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 462;
                  break;
               case 463:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 48)
                     kind = 48;
                  jjstateSet[jjnewStateCnt++] = 463;
                  break;
               case 465:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 464;
                  break;
               case 474:
                  if (curChar == 58)
                     jjAddStates(155, 159);
                  break;
               case 476:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 477;
                  break;
               case 478:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 67)
                     kind = 67;
                  jjstateSet[jjnewStateCnt++] = 478;
                  break;
               case 498:
                  if (curChar == 58)
                     jjAddStates(160, 162);
                  break;
               case 500:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 501;
                  break;
               case 502:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 71)
                     kind = 71;
                  jjstateSet[jjnewStateCnt++] = 502;
                  break;
               case 514:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 513;
                  break;
               case 525:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 526;
                  break;
               case 527:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(163, 164);
                  break;
               case 528:
                  if (curChar != 58)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(529, 531);
                  break;
               case 530:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(530, 531);
                  break;
               case 531:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 532;
                  break;
               case 533:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(531, 533);
                  break;
               case 535:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 534;
                  break;
               case 544:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 545;
                  break;
               case 546:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(165, 166);
                  break;
               case 547:
                  if (curChar == 58)
                     jjAddStates(167, 169);
                  break;
               case 548:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(170, 173);
                  break;
               case 549:
                  if (curChar == 46)
                     jjCheckNAdd(550);
                  break;
               case 550:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(550, 551);
                  break;
               case 551:
                  if (curChar != 58)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(552, 554);
                  break;
               case 553:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(553, 554);
                  break;
               case 554:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 555;
                  break;
               case 556:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(554, 556);
                  break;
               case 557:
                  if (curChar == 46)
                     jjCheckNAdd(558);
                  break;
               case 558:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(174, 176);
                  break;
               case 560:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(177, 178);
                  break;
               case 561:
                  if (curChar == 46)
                     jjCheckNAdd(562);
                  break;
               case 562:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(562, 551);
                  break;
               case 563:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(179, 182);
                  break;
               case 564:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(564, 551);
                  break;
               case 565:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(183, 185);
                  break;
               case 566:
                  if (curChar == 46)
                     jjCheckNAdd(567);
                  break;
               case 567:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(567, 551);
                  break;
               case 568:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(186, 190);
                  break;
               case 569:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(191, 193);
                  break;
               case 570:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(194, 197);
                  break;
               case 571:
                  if (curChar == 46)
                     jjCheckNAdd(572);
                  break;
               case 572:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(198, 200);
                  break;
               case 573:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(201, 204);
                  break;
               case 574:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(574, 551);
                  break;
               case 575:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(205, 207);
                  break;
               case 576:
                  if (curChar == 46)
                     jjCheckNAdd(577);
                  break;
               case 577:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(577, 551);
                  break;
               case 578:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(550, 558);
                  break;
               case 579:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(208, 215);
                  break;
               case 581:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 580;
                  break;
               case 591:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 596;
                  break;
               case 602:
                  if (curChar == 58)
                     jjAddStates(216, 218);
                  break;
               case 604:
                  if (curChar == 58)
                     jjAddStates(219, 221);
                  break;
               case 677:
                  if (curChar != 58)
                     break;
                  if (kind > 76)
                     kind = 76;
                  jjCheckNAddTwoStates(678, 679);
                  break;
               case 678:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 76)
                     kind = 76;
                  jjCheckNAddTwoStates(678, 679);
                  break;
               case 679:
                  if (curChar == 58)
                     jjCheckNAdd(680);
                  break;
               case 680:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 76)
                     kind = 76;
                  jjCheckNAddTwoStates(679, 680);
                  break;
               case 687:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 686;
                  break;
               case 697:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 696;
                  break;
               case 710:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 709;
                  break;
               case 725:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 724;
                  break;
               case 740:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 739;
                  break;
               case 752:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 751;
                  break;
               case 763:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 762;
                  break;
               case 770:
                  if (curChar != 58)
                     break;
                  if (kind > 77)
                     kind = 77;
                  jjCheckNAddTwoStates(771, 776);
                  break;
               case 771:
                  if (curChar == 34)
                     jjCheckNAddStates(222, 224);
                  break;
               case 772:
                  if ((0xfffffffbffffdbffL & l) != 0L)
                     jjCheckNAddStates(222, 224);
                  break;
               case 774:
                  if ((0xffffffffffffdbffL & l) != 0L)
                     jjCheckNAddStates(222, 224);
                  break;
               case 775:
                  if (curChar != 34)
                     break;
                  if (kind > 77)
                     kind = 77;
                  jjCheckNAdd(776);
                  break;
               case 776:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 777;
                  break;
               case 777:
                  if (curChar == 34)
                     jjCheckNAddStates(225, 227);
                  break;
               case 778:
                  if ((0xfffffffbffffdbffL & l) != 0L)
                     jjCheckNAddStates(225, 227);
                  break;
               case 780:
                  if ((0xffffffffffffdbffL & l) != 0L)
                     jjCheckNAddStates(225, 227);
                  break;
               case 782:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 781;
                  break;
               case 796:
                  if (curChar == 58)
                     jjAddStates(228, 232);
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      else if (curChar < 128)
      {
         long l = 1L << (curChar & 077);
         do
         {
            switch(jjstateSet[--i])
            {
               case 3:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                  {
                     if (kind > 55)
                        kind = 55;
                     jjCheckNAddTwoStates(8, 9);
                  }
                  if (curChar == 109)
                     jjAddStates(233, 236);
                  else if (curChar == 100)
                     jjAddStates(237, 238);
                  else if (curChar == 112)
                     jjAddStates(239, 248);
                  else if (curChar == 97)
                     jjAddStates(249, 258);
                  else if (curChar == 99)
                     jjAddStates(259, 269);
                  else if (curChar == 110)
                     jjAddStates(270, 275);
                  else if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 11;
                  else if (curChar == 102)
                     jjstateSet[jjnewStateCnt++] = 6;
                  else if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 2;
                  break;
               case 0:
                  if (curChar == 101 && kind > 42)
                     kind = 42;
                  break;
               case 1:
                  if (curChar == 117)
                     jjCheckNAdd(0);
                  break;
               case 2:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 1;
                  break;
               case 4:
                  if (curChar == 115)
                     jjCheckNAdd(0);
                  break;
               case 5:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 4;
                  break;
               case 6:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 5;
                  break;
               case 7:
                  if (curChar == 102)
                     jjstateSet[jjnewStateCnt++] = 6;
                  break;
               case 8:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjCheckNAddTwoStates(8, 9);
                  break;
               case 9:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjCheckNAdd(9);
                  break;
               case 10:
                  if (curChar == 109 && kind > 73)
                     kind = 73;
                  break;
               case 11:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 10;
                  break;
               case 12:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 11;
                  break;
               case 14:
                  if ((0xffffffffefffffffL & l) != 0L)
                     jjCheckNAddStates(13, 15);
                  break;
               case 15:
                  if (curChar == 92)
                     jjstateSet[jjnewStateCnt++] = 16;
                  break;
               case 16:
                  jjCheckNAddStates(13, 15);
                  break;
               case 23:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(276, 278);
                  break;
               case 46:
                  if (kind > 5)
                     kind = 5;
                  jjstateSet[jjnewStateCnt++] = 46;
                  break;
               case 48:
                  jjCheckNAddTwoStates(48, 49);
                  break;
               case 50:
               case 51:
                  jjCheckNAddTwoStates(51, 49);
                  break;
               case 53:
                  if (curChar == 110)
                     jjAddStates(270, 275);
                  break;
               case 54:
                  if (curChar == 108 && kind > 42)
                     kind = 42;
                  break;
               case 55:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 54;
                  break;
               case 56:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 55;
                  break;
               case 57:
                  if (curChar == 115 && kind > 69)
                     kind = 69;
                  break;
               case 58:
                  if (curChar == 108)
                     jjCheckNAdd(57);
                  break;
               case 59:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 58;
                  break;
               case 60:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 59;
                  break;
               case 61:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 60;
                  break;
               case 62:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 61;
                  break;
               case 63:
                  if (curChar == 112)
                     jjstateSet[jjnewStateCnt++] = 62;
                  break;
               case 64:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 63;
                  break;
               case 65:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 64;
                  break;
               case 66:
               case 829:
                  if (curChar == 121)
                     jjCheckNAdd(57);
                  break;
               case 67:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 66;
                  break;
               case 68:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 67;
                  break;
               case 69:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 68;
                  break;
               case 70:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 69;
                  break;
               case 71:
                  if (curChar == 116)
                     jjCheckNAdd(57);
                  break;
               case 72:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 71;
                  break;
               case 73:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 72;
                  break;
               case 74:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 73;
                  break;
               case 75:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 74;
                  break;
               case 76:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 75;
                  break;
               case 77:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 76;
                  break;
               case 78:
               case 84:
               case 823:
                  if (curChar == 114)
                     jjCheckNAdd(57);
                  break;
               case 79:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 78;
                  break;
               case 80:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 79;
                  break;
               case 81:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 80;
                  break;
               case 82:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 81;
                  break;
               case 83:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 82;
                  break;
               case 85:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 84;
                  break;
               case 86:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 85;
                  break;
               case 87:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 86;
                  break;
               case 88:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 87;
                  break;
               case 89:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 88;
                  break;
               case 90:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 89;
                  break;
               case 91:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 90;
                  break;
               case 92:
                  if (curChar == 99)
                     jjAddStates(259, 269);
                  break;
               case 94:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjCheckNAddTwoStates(94, 95);
                  break;
               case 95:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjCheckNAdd(95);
                  break;
               case 96:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 93;
                  break;
               case 97:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 96;
                  break;
               case 98:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 97;
                  break;
               case 99:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 98;
                  break;
               case 100:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 99;
                  break;
               case 101:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 100;
                  break;
               case 102:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 101;
                  break;
               case 104:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(279, 281);
                  break;
               case 105:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(105, 120);
                  break;
               case 107:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 49)
                     kind = 49;
                  jjCheckNAddTwoStates(107, 108);
                  break;
               case 108:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 49)
                     kind = 49;
                  jjCheckNAdd(108);
                  break;
               case 109:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 106;
                  break;
               case 111:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 110;
                  break;
               case 112:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 111;
                  break;
               case 113:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 112;
                  break;
               case 114:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 113;
                  break;
               case 115:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 114;
                  break;
               case 116:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 115;
                  break;
               case 117:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 116;
                  break;
               case 118:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 117;
                  break;
               case 119:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 118;
                  break;
               case 121:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 103;
                  break;
               case 122:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 121;
                  break;
               case 123:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 122;
                  break;
               case 124:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 123;
                  break;
               case 125:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 124;
                  break;
               case 126:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 125;
                  break;
               case 127:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 126;
                  break;
               case 129:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(282, 284);
                  break;
               case 130:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(130, 145);
                  break;
               case 132:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjCheckNAddTwoStates(132, 133);
                  break;
               case 133:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjCheckNAdd(133);
                  break;
               case 134:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 131;
                  break;
               case 136:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 135;
                  break;
               case 137:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 136;
                  break;
               case 138:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 137;
                  break;
               case 139:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 138;
                  break;
               case 140:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 139;
                  break;
               case 141:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 140;
                  break;
               case 142:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 141;
                  break;
               case 143:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 142;
                  break;
               case 144:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 143;
                  break;
               case 146:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 128;
                  break;
               case 147:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 146;
                  break;
               case 148:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 147;
                  break;
               case 149:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 148;
                  break;
               case 150:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 149;
                  break;
               case 151:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 150;
                  break;
               case 152:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 151;
                  break;
               case 154:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(285, 287);
                  break;
               case 155:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(155, 170);
                  break;
               case 157:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 51)
                     kind = 51;
                  jjCheckNAddTwoStates(157, 158);
                  break;
               case 158:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 51)
                     kind = 51;
                  jjCheckNAdd(158);
                  break;
               case 159:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 156;
                  break;
               case 161:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 160;
                  break;
               case 162:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 161;
                  break;
               case 163:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 162;
                  break;
               case 164:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 163;
                  break;
               case 165:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 164;
                  break;
               case 166:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 165;
                  break;
               case 167:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 166;
                  break;
               case 168:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 167;
                  break;
               case 169:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 168;
                  break;
               case 171:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 153;
                  break;
               case 172:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 171;
                  break;
               case 173:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 172;
                  break;
               case 174:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 173;
                  break;
               case 175:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 174;
                  break;
               case 176:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 175;
                  break;
               case 177:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 176;
                  break;
               case 179:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(288, 290);
                  break;
               case 180:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(180, 195);
                  break;
               case 182:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 52)
                     kind = 52;
                  jjCheckNAddTwoStates(182, 183);
                  break;
               case 183:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 52)
                     kind = 52;
                  jjCheckNAdd(183);
                  break;
               case 184:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 181;
                  break;
               case 186:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 185;
                  break;
               case 187:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 186;
                  break;
               case 188:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 187;
                  break;
               case 189:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 188;
                  break;
               case 190:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 189;
                  break;
               case 191:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 190;
                  break;
               case 192:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 191;
                  break;
               case 193:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 192;
                  break;
               case 194:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 193;
                  break;
               case 196:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 178;
                  break;
               case 197:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 196;
                  break;
               case 198:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 197;
                  break;
               case 199:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 198;
                  break;
               case 200:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 199;
                  break;
               case 201:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 200;
                  break;
               case 202:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 201;
                  break;
               case 204:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(291, 293);
                  break;
               case 205:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(205, 220);
                  break;
               case 207:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 53)
                     kind = 53;
                  jjCheckNAddTwoStates(207, 208);
                  break;
               case 208:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 53)
                     kind = 53;
                  jjCheckNAdd(208);
                  break;
               case 209:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 206;
                  break;
               case 211:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 210;
                  break;
               case 212:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 211;
                  break;
               case 213:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 212;
                  break;
               case 214:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 213;
                  break;
               case 215:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 214;
                  break;
               case 216:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 215;
                  break;
               case 217:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 216;
                  break;
               case 218:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 217;
                  break;
               case 219:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 218;
                  break;
               case 221:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 203;
                  break;
               case 222:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 221;
                  break;
               case 223:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 222;
                  break;
               case 224:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 223;
                  break;
               case 225:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 224;
                  break;
               case 226:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 225;
                  break;
               case 227:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 226;
                  break;
               case 229:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 70)
                     kind = 70;
                  jjCheckNAddTwoStates(229, 230);
                  break;
               case 230:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 70)
                     kind = 70;
                  jjCheckNAdd(230);
                  break;
               case 231:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 228;
                  break;
               case 232:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 231;
                  break;
               case 233:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 232;
                  break;
               case 235:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 234;
                  break;
               case 236:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 235;
                  break;
               case 237:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 236;
                  break;
               case 238:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 237;
                  break;
               case 239:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 238;
                  break;
               case 240:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 239;
                  break;
               case 241:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 240;
                  break;
               case 243:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(294, 296);
                  break;
               case 244:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(244, 271);
                  break;
               case 246:
                  if (curChar == 109)
                     jjCheckNAdd(247);
                  break;
               case 248:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 72)
                     kind = 72;
                  jjCheckNAddTwoStates(248, 249);
                  break;
               case 249:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 72)
                     kind = 72;
                  jjCheckNAdd(249);
                  break;
               case 250:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 246;
                  break;
               case 251:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 250;
                  break;
               case 252:
                  if (curChar == 103)
                     jjCheckNAdd(247);
                  break;
               case 253:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 252;
                  break;
               case 254:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 253;
                  break;
               case 255:
                  if (curChar == 109)
                     jjAddStates(297, 298);
                  break;
               case 256:
                  if (curChar == 110)
                     jjCheckNAdd(247);
                  break;
               case 257:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 256;
                  break;
               case 258:
                  if (curChar == 120)
                     jjCheckNAdd(247);
                  break;
               case 259:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 258;
                  break;
               case 260:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 245;
                  break;
               case 262:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 261;
                  break;
               case 263:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 262;
                  break;
               case 264:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 263;
                  break;
               case 265:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 264;
                  break;
               case 266:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 265;
                  break;
               case 267:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 266;
                  break;
               case 268:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 267;
                  break;
               case 269:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 268;
                  break;
               case 270:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 269;
                  break;
               case 272:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 242;
                  break;
               case 273:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 272;
                  break;
               case 274:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 273;
                  break;
               case 275:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 274;
                  break;
               case 276:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 275;
                  break;
               case 277:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 276;
                  break;
               case 278:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 277;
                  break;
               case 284:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddStates(299, 301);
                  break;
               case 285:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddTwoStates(285, 286);
                  break;
               case 287:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddStates(302, 304);
                  break;
               case 288:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 78)
                     kind = 78;
                  jjCheckNAddTwoStates(286, 288);
                  break;
               case 291:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(305, 307);
                  break;
               case 312:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 279;
                  break;
               case 313:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 312;
                  break;
               case 314:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 313;
                  break;
               case 315:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 314;
                  break;
               case 316:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 315;
                  break;
               case 317:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 316;
                  break;
               case 318:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 317;
                  break;
               case 320:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(308, 310);
                  break;
               case 321:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(321, 322);
                  break;
               case 324:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(311, 313);
                  break;
               case 325:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(325, 326);
                  break;
               case 327:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddStates(314, 316);
                  break;
               case 328:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(328, 329);
                  break;
               case 330:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddStates(317, 319);
                  break;
               case 331:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(329, 331);
                  break;
               case 332:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 323;
                  break;
               case 334:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 333;
                  break;
               case 335:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 334;
                  break;
               case 336:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 335;
                  break;
               case 337:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 336;
                  break;
               case 338:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 337;
                  break;
               case 339:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 338;
                  break;
               case 340:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 339;
                  break;
               case 341:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 340;
                  break;
               case 342:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 341;
                  break;
               case 343:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 319;
                  break;
               case 344:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 343;
                  break;
               case 345:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 344;
                  break;
               case 346:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 345;
                  break;
               case 347:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 346;
                  break;
               case 348:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 347;
                  break;
               case 349:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 348;
                  break;
               case 351:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(320, 322);
                  break;
               case 352:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(352, 353);
                  break;
               case 355:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(323, 325);
                  break;
               case 356:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(356, 357);
                  break;
               case 362:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddStates(326, 328);
                  break;
               case 363:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(363, 364);
                  break;
               case 365:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddStates(329, 331);
                  break;
               case 366:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 80)
                     kind = 80;
                  jjCheckNAddTwoStates(364, 366);
                  break;
               case 369:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(332, 334);
                  break;
               case 390:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 354;
                  break;
               case 392:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 391;
                  break;
               case 393:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 392;
                  break;
               case 394:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 393;
                  break;
               case 395:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 394;
                  break;
               case 396:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 395;
                  break;
               case 397:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 396;
                  break;
               case 398:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 397;
                  break;
               case 399:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 398;
                  break;
               case 400:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 399;
                  break;
               case 401:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 350;
                  break;
               case 402:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 401;
                  break;
               case 403:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 402;
                  break;
               case 404:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 403;
                  break;
               case 405:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 404;
                  break;
               case 406:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 405;
                  break;
               case 407:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 406;
                  break;
               case 408:
                  if (curChar == 97)
                     jjAddStates(249, 258);
                  break;
               case 410:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjCheckNAddTwoStates(410, 411);
                  break;
               case 411:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjCheckNAdd(411);
                  break;
               case 412:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 409;
                  break;
               case 414:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 413;
                  break;
               case 415:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 414;
                  break;
               case 416:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 415;
                  break;
               case 417:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 416;
                  break;
               case 418:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 417;
                  break;
               case 419:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 418;
                  break;
               case 420:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 419;
                  break;
               case 421:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 420;
                  break;
               case 423:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjCheckNAddTwoStates(423, 424);
                  break;
               case 424:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjCheckNAdd(424);
                  break;
               case 425:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 422;
                  break;
               case 427:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 426;
                  break;
               case 428:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 427;
                  break;
               case 429:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 428;
                  break;
               case 430:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 429;
                  break;
               case 431:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 430;
                  break;
               case 432:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 431;
                  break;
               case 433:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 432;
                  break;
               case 434:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 433;
                  break;
               case 436:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 46)
                     kind = 46;
                  jjCheckNAddTwoStates(436, 437);
                  break;
               case 437:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 46)
                     kind = 46;
                  jjCheckNAdd(437);
                  break;
               case 438:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 435;
                  break;
               case 440:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 439;
                  break;
               case 441:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 440;
                  break;
               case 442:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 441;
                  break;
               case 443:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 442;
                  break;
               case 444:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 443;
                  break;
               case 445:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 444;
                  break;
               case 446:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 445;
                  break;
               case 447:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 446;
                  break;
               case 449:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 47)
                     kind = 47;
                  jjCheckNAddTwoStates(449, 450);
                  break;
               case 450:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 47)
                     kind = 47;
                  jjCheckNAdd(450);
                  break;
               case 451:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 448;
                  break;
               case 453:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 452;
                  break;
               case 454:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 453;
                  break;
               case 455:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 454;
                  break;
               case 456:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 455;
                  break;
               case 457:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 456;
                  break;
               case 458:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 457;
                  break;
               case 459:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 458;
                  break;
               case 460:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 459;
                  break;
               case 462:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 48)
                     kind = 48;
                  jjCheckNAddTwoStates(462, 463);
                  break;
               case 463:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 48)
                     kind = 48;
                  jjCheckNAdd(463);
                  break;
               case 464:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 461;
                  break;
               case 466:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 465;
                  break;
               case 467:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 466;
                  break;
               case 468:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 467;
                  break;
               case 469:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 468;
                  break;
               case 470:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 469;
                  break;
               case 471:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 470;
                  break;
               case 472:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 471;
                  break;
               case 473:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 472;
                  break;
               case 475:
                  if (curChar == 65)
                     jjCheckNAdd(476);
                  break;
               case 477:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 67)
                     kind = 67;
                  jjCheckNAddTwoStates(477, 478);
                  break;
               case 478:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 67)
                     kind = 67;
                  jjCheckNAdd(478);
                  break;
               case 479:
                  if (curChar == 67)
                     jjCheckNAdd(475);
                  break;
               case 480:
                  if (curChar == 66)
                     jjCheckNAdd(475);
                  break;
               case 481:
                  if (curChar == 78)
                     jjCheckNAdd(476);
                  break;
               case 482:
                  if (curChar == 73)
                     jjstateSet[jjnewStateCnt++] = 481;
                  break;
               case 483:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 482;
                  break;
               case 484:
                  if (curChar == 68)
                     jjstateSet[jjnewStateCnt++] = 483;
                  break;
               case 485:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 484;
                  break;
               case 486:
                  if (curChar == 84)
                     jjCheckNAdd(476);
                  break;
               case 487:
                  if (curChar == 71)
                     jjstateSet[jjnewStateCnt++] = 486;
                  break;
               case 488:
                  if (curChar == 78)
                     jjstateSet[jjnewStateCnt++] = 487;
                  break;
               case 489:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 488;
                  break;
               case 490:
                  if (curChar == 76)
                     jjCheckNAdd(476);
                  break;
               case 491:
                  if (curChar == 82)
                     jjstateSet[jjnewStateCnt++] = 490;
                  break;
               case 492:
                  if (curChar == 84)
                     jjstateSet[jjnewStateCnt++] = 491;
                  break;
               case 493:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 492;
                  break;
               case 494:
                  if (curChar == 80)
                     jjstateSet[jjnewStateCnt++] = 493;
                  break;
               case 495:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 474;
                  break;
               case 496:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 495;
                  break;
               case 497:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 496;
                  break;
               case 499:
                  if (curChar == 109)
                     jjCheckNAdd(500);
                  break;
               case 501:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 71)
                     kind = 71;
                  jjCheckNAddTwoStates(501, 502);
                  break;
               case 502:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 71)
                     kind = 71;
                  jjCheckNAdd(502);
                  break;
               case 503:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 499;
                  break;
               case 504:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 503;
                  break;
               case 505:
                  if (curChar == 103)
                     jjCheckNAdd(500);
                  break;
               case 506:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 505;
                  break;
               case 507:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 506;
                  break;
               case 508:
                  if (curChar == 109)
                     jjAddStates(335, 336);
                  break;
               case 509:
                  if (curChar == 110)
                     jjCheckNAdd(500);
                  break;
               case 510:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 509;
                  break;
               case 511:
                  if (curChar == 120)
                     jjCheckNAdd(500);
                  break;
               case 512:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 511;
                  break;
               case 513:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 498;
                  break;
               case 515:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 514;
                  break;
               case 516:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 515;
                  break;
               case 517:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 516;
                  break;
               case 518:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 517;
                  break;
               case 519:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 518;
                  break;
               case 520:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 519;
                  break;
               case 521:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 520;
                  break;
               case 522:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 521;
                  break;
               case 523:
                  if (curChar == 103 && kind > 73)
                     kind = 73;
                  break;
               case 524:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 523;
                  break;
               case 526:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(337, 339);
                  break;
               case 527:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(527, 528);
                  break;
               case 529:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddStates(340, 342);
                  break;
               case 530:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(530, 531);
                  break;
               case 532:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddStates(343, 345);
                  break;
               case 533:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(531, 533);
                  break;
               case 534:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 525;
                  break;
               case 536:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 535;
                  break;
               case 537:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 536;
                  break;
               case 538:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 537;
                  break;
               case 539:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 538;
                  break;
               case 540:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 539;
                  break;
               case 541:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 540;
                  break;
               case 542:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 541;
                  break;
               case 543:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 542;
                  break;
               case 545:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(346, 348);
                  break;
               case 546:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(546, 547);
                  break;
               case 552:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddStates(349, 351);
                  break;
               case 553:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(553, 554);
                  break;
               case 555:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddStates(352, 354);
                  break;
               case 556:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 79)
                     kind = 79;
                  jjCheckNAddTwoStates(554, 556);
                  break;
               case 559:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(355, 357);
                  break;
               case 580:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 544;
                  break;
               case 582:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 581;
                  break;
               case 583:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 582;
                  break;
               case 584:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 583;
                  break;
               case 585:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 584;
                  break;
               case 586:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 585;
                  break;
               case 587:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 586;
                  break;
               case 588:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 587;
                  break;
               case 589:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 588;
                  break;
               case 590:
                  if (curChar == 112)
                     jjAddStates(239, 248);
                  break;
               case 592:
                  if (curChar == 100 && kind > 54)
                     kind = 54;
                  break;
               case 593:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 592;
                  break;
               case 594:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 593;
                  break;
               case 595:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 594;
                  break;
               case 596:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 595;
                  break;
               case 597:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 591;
                  break;
               case 598:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 597;
                  break;
               case 599:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 598;
                  break;
               case 600:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 599;
                  break;
               case 601:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 600;
                  break;
               case 603:
                  if (curChar == 109)
                     jjCheckNAdd(604);
                  break;
               case 605:
                  if (curChar == 101 && kind > 68)
                     kind = 68;
                  break;
               case 606:
                  if (curChar == 99)
                     jjstateSet[jjnewStateCnt++] = 605;
                  break;
               case 607:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 606;
                  break;
               case 608:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 607;
                  break;
               case 609:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 608;
                  break;
               case 610:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 609;
                  break;
               case 611:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 610;
                  break;
               case 612:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 611;
                  break;
               case 613:
                  if (curChar == 110)
                     jjAddStates(358, 362);
                  break;
               case 614:
                  if (curChar == 115 && kind > 68)
                     kind = 68;
                  break;
               case 615:
                  if (curChar == 108)
                     jjCheckNAdd(614);
                  break;
               case 616:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 615;
                  break;
               case 617:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 616;
                  break;
               case 618:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 617;
                  break;
               case 619:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 618;
                  break;
               case 620:
                  if (curChar == 112)
                     jjstateSet[jjnewStateCnt++] = 619;
                  break;
               case 621:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 620;
                  break;
               case 622:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 621;
                  break;
               case 623:
               case 656:
                  if (curChar == 121)
                     jjCheckNAdd(614);
                  break;
               case 624:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 623;
                  break;
               case 625:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 624;
                  break;
               case 626:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 625;
                  break;
               case 627:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 626;
                  break;
               case 628:
                  if (curChar == 116)
                     jjCheckNAdd(614);
                  break;
               case 629:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 628;
                  break;
               case 630:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 629;
                  break;
               case 631:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 630;
                  break;
               case 632:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 631;
                  break;
               case 633:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 632;
                  break;
               case 634:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 633;
                  break;
               case 635:
               case 641:
               case 650:
                  if (curChar == 114)
                     jjCheckNAdd(614);
                  break;
               case 636:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 635;
                  break;
               case 637:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 636;
                  break;
               case 638:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 637;
                  break;
               case 639:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 638;
                  break;
               case 640:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 639;
                  break;
               case 642:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 641;
                  break;
               case 643:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 642;
                  break;
               case 644:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 643;
                  break;
               case 645:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 644;
                  break;
               case 646:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 645;
                  break;
               case 647:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 646;
                  break;
               case 648:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 647;
                  break;
               case 649:
                  if (curChar == 109)
                     jjAddStates(363, 364);
                  break;
               case 651:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 650;
                  break;
               case 652:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 651;
                  break;
               case 653:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 652;
                  break;
               case 654:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 653;
                  break;
               case 655:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 654;
                  break;
               case 657:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 656;
                  break;
               case 658:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 657;
                  break;
               case 659:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 658;
                  break;
               case 660:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 659;
                  break;
               case 661:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 603;
                  break;
               case 662:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 661;
                  break;
               case 663:
                  if (curChar == 103)
                     jjCheckNAdd(604);
                  break;
               case 664:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 663;
                  break;
               case 665:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 664;
                  break;
               case 666:
                  if (curChar == 109)
                     jjAddStates(365, 366);
                  break;
               case 667:
                  if (curChar == 110)
                     jjCheckNAdd(604);
                  break;
               case 668:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 667;
                  break;
               case 669:
                  if (curChar == 120)
                     jjCheckNAdd(604);
                  break;
               case 670:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 669;
                  break;
               case 671:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 602;
                  break;
               case 672:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 671;
                  break;
               case 673:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 672;
                  break;
               case 674:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 673;
                  break;
               case 675:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 674;
                  break;
               case 676:
                  if (curChar == 110)
                     jjCheckNAdd(677);
                  break;
               case 678:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 76)
                     kind = 76;
                  jjCheckNAddTwoStates(678, 679);
                  break;
               case 680:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 76)
                     kind = 76;
                  jjCheckNAddTwoStates(679, 680);
                  break;
               case 681:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 676;
                  break;
               case 682:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 681;
                  break;
               case 683:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 682;
                  break;
               case 684:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 683;
                  break;
               case 685:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 684;
                  break;
               case 686:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 685;
                  break;
               case 688:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 687;
                  break;
               case 689:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 688;
                  break;
               case 690:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 689;
                  break;
               case 691:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 690;
                  break;
               case 692:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 691;
                  break;
               case 693:
                  if (curChar == 109)
                     jjCheckNAdd(677);
                  break;
               case 694:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 693;
                  break;
               case 695:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 694;
                  break;
               case 696:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 695;
                  break;
               case 698:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 697;
                  break;
               case 699:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 698;
                  break;
               case 700:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 699;
                  break;
               case 701:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 700;
                  break;
               case 702:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 701;
                  break;
               case 703:
                  if (curChar == 101)
                     jjCheckNAdd(677);
                  break;
               case 704:
                  if (curChar == 116)
                     jjCheckNAdd(703);
                  break;
               case 705:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 704;
                  break;
               case 706:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 705;
                  break;
               case 707:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 706;
                  break;
               case 708:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 707;
                  break;
               case 709:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 708;
                  break;
               case 711:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 710;
                  break;
               case 712:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 711;
                  break;
               case 713:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 712;
                  break;
               case 714:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 713;
                  break;
               case 715:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 714;
                  break;
               case 716:
                  if (curChar == 112)
                     jjCheckNAdd(703);
                  break;
               case 717:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 716;
                  break;
               case 718:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 717;
                  break;
               case 719:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 718;
                  break;
               case 720:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 719;
                  break;
               case 721:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 720;
                  break;
               case 722:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 721;
                  break;
               case 723:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 722;
                  break;
               case 724:
                  if (curChar == 112)
                     jjstateSet[jjnewStateCnt++] = 723;
                  break;
               case 726:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 725;
                  break;
               case 727:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 726;
                  break;
               case 728:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 727;
                  break;
               case 729:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 728;
                  break;
               case 730:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 729;
                  break;
               case 731:
                  if (curChar == 116)
                     jjCheckNAdd(677);
                  break;
               case 732:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 731;
                  break;
               case 733:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 732;
                  break;
               case 734:
                  if (curChar == 112)
                     jjstateSet[jjnewStateCnt++] = 733;
                  break;
               case 735:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 734;
                  break;
               case 736:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 735;
                  break;
               case 737:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 736;
                  break;
               case 738:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 737;
                  break;
               case 739:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 738;
                  break;
               case 741:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 740;
                  break;
               case 742:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 741;
                  break;
               case 743:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 742;
                  break;
               case 744:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 743;
                  break;
               case 745:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 744;
                  break;
               case 746:
                  if (curChar == 114)
                     jjCheckNAdd(677);
                  break;
               case 747:
               case 758:
                  if (curChar == 101)
                     jjCheckNAdd(746);
                  break;
               case 748:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 747;
                  break;
               case 749:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 748;
                  break;
               case 750:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 749;
                  break;
               case 751:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 750;
                  break;
               case 753:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 752;
                  break;
               case 754:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 753;
                  break;
               case 755:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 754;
                  break;
               case 756:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 755;
                  break;
               case 757:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 756;
                  break;
               case 759:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 758;
                  break;
               case 760:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 759;
                  break;
               case 761:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 760;
                  break;
               case 762:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 761;
                  break;
               case 764:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 763;
                  break;
               case 765:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 764;
                  break;
               case 766:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 765;
                  break;
               case 767:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 766;
                  break;
               case 768:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 767;
                  break;
               case 769:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 770;
                  break;
               case 772:
                  if ((0xffffffffefffffffL & l) != 0L)
                     jjCheckNAddStates(222, 224);
                  break;
               case 773:
                  if (curChar == 92)
                     jjstateSet[jjnewStateCnt++] = 774;
                  break;
               case 774:
                  jjCheckNAddStates(222, 224);
                  break;
               case 778:
                  if ((0xffffffffefffffffL & l) != 0L)
                     jjCheckNAddStates(225, 227);
                  break;
               case 779:
                  if (curChar == 92)
                     jjstateSet[jjnewStateCnt++] = 780;
                  break;
               case 780:
                  jjCheckNAddStates(225, 227);
                  break;
               case 781:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 769;
                  break;
               case 783:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 782;
                  break;
               case 784:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 783;
                  break;
               case 785:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 784;
                  break;
               case 786:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 785;
                  break;
               case 787:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 786;
                  break;
               case 788:
                  if (curChar == 100)
                     jjAddStates(237, 238);
                  break;
               case 789:
                  if (curChar == 101 && kind > 69)
                     kind = 69;
                  break;
               case 790:
                  if (curChar == 99)
                     jjstateSet[jjnewStateCnt++] = 789;
                  break;
               case 791:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 790;
                  break;
               case 792:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 791;
                  break;
               case 793:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 792;
                  break;
               case 794:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 793;
                  break;
               case 795:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 794;
                  break;
               case 797:
                  if (curChar == 121 && kind > 74)
                     kind = 74;
                  break;
               case 798:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 797;
                  break;
               case 799:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 798;
                  break;
               case 800:
                  if (curChar == 107 && kind > 74)
                     kind = 74;
                  break;
               case 801:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 800;
                  break;
               case 802:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 801;
                  break;
               case 803:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 802;
                  break;
               case 804:
                  if (curChar == 104 && kind > 74)
                     kind = 74;
                  break;
               case 805:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 804;
                  break;
               case 806:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 805;
                  break;
               case 807:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 806;
                  break;
               case 808:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 807;
                  break;
               case 809:
                  if (curChar == 114 && kind > 74)
                     kind = 74;
                  break;
               case 810:
                  if (curChar == 101)
                     jjCheckNAdd(809);
                  break;
               case 811:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 810;
                  break;
               case 812:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 811;
                  break;
               case 813:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 812;
                  break;
               case 814:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 813;
                  break;
               case 815:
                  if (curChar == 113)
                     jjstateSet[jjnewStateCnt++] = 814;
                  break;
               case 816:
                  if (curChar == 97)
                     jjCheckNAdd(809);
                  break;
               case 817:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 816;
                  break;
               case 818:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 817;
                  break;
               case 819:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 796;
                  break;
               case 820:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 819;
                  break;
               case 821:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 820;
                  break;
               case 822:
                  if (curChar == 109)
                     jjAddStates(233, 236);
                  break;
               case 824:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 823;
                  break;
               case 825:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 824;
                  break;
               case 826:
                  if (curChar == 104)
                     jjstateSet[jjnewStateCnt++] = 825;
                  break;
               case 827:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 826;
                  break;
               case 828:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 827;
                  break;
               case 830:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 829;
                  break;
               case 831:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 830;
                  break;
               case 832:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 831;
                  break;
               case 833:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 832;
                  break;
               case 834:
                  if (curChar == 110 && kind > 73)
                     kind = 73;
                  break;
               case 835:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 834;
                  break;
               case 836:
                  if (curChar == 120 && kind > 73)
                     kind = 73;
                  break;
               case 837:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 836;
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      else
      {
         int i2 = (curChar & 0xff) >> 6;
         long l2 = 1L << (curChar & 077);
         do
         {
            switch(jjstateSet[--i])
            {
               case 14:
               case 16:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddStates(13, 15);
                  break;
               case 46:
                  if ((jjbitVec0[i2] & l2) == 0L)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjstateSet[jjnewStateCnt++] = 46;
                  break;
               case 48:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddTwoStates(48, 49);
                  break;
               case 50:
               case 51:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddTwoStates(51, 49);
                  break;
               case 772:
               case 774:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddStates(222, 224);
                  break;
               case 778:
               case 780:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddStates(225, 227);
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      if (kind != 0x7fffffff)
      {
         jjmatchedKind = kind;
         jjmatchedPos = curPos;
         kind = 0x7fffffff;
      }
      ++curPos;
      if ((i = jjnewStateCnt) == (startsAt = 838 - (jjnewStateCnt = startsAt)))
         break;
      try { curChar = input_stream.readChar(); }
      catch(java.io.IOException e) { break; }
   }
   if (jjmatchedPos > strPos)
      return curPos;

   int toRet = Math.max(curPos, seenUpto);

   if (curPos < toRet)
      for (i = toRet - Math.min(curPos, seenUpto); i-- > 0; )
         try { curChar = input_stream.readChar(); }
         catch(java.io.IOException e) { throw new Error("Internal Error : Please send a bug report."); }

   if (jjmatchedPos < strPos)
   {
      jjmatchedKind = strKind;
      jjmatchedPos = strPos;
   }
   else if (jjmatchedPos == strPos && jjmatchedKind > strKind)
      jjmatchedKind = strKind;

   return toRet;
}
static final int[] jjnextStates = {
   38, 39, 40, 33, 34, 35, 23, 19, 21, 32, 37, 45, 47, 14, 15, 17, 
   25, 27, 28, 29, 30, 33, 34, 35, 23, 34, 35, 23, 38, 39, 40, 49, 
   50, 52, 105, 120, 130, 145, 155, 170, 180, 195, 205, 220, 244, 271, 251, 254, 
   255, 280, 310, 311, 281, 289, 300, 305, 290, 291, 283, 293, 295, 296, 297, 298, 
   283, 297, 298, 283, 301, 302, 303, 291, 283, 301, 291, 283, 302, 303, 291, 283, 
   304, 291, 283, 306, 307, 308, 283, 307, 308, 283, 306, 307, 308, 301, 302, 303, 
   291, 283, 321, 322, 325, 326, 352, 353, 356, 357, 358, 388, 389, 359, 367, 378, 
   383, 368, 369, 361, 371, 373, 374, 375, 376, 361, 375, 376, 361, 379, 380, 381, 
   369, 361, 379, 369, 361, 380, 381, 369, 361, 382, 369, 361, 384, 385, 386, 361, 
   385, 386, 361, 384, 385, 386, 379, 380, 381, 369, 361, 479, 480, 485, 489, 494, 
   504, 507, 508, 527, 528, 546, 547, 548, 578, 579, 549, 557, 568, 573, 558, 559, 
   551, 561, 563, 564, 565, 566, 551, 565, 566, 551, 569, 570, 571, 559, 551, 569, 
   559, 551, 570, 571, 559, 551, 572, 559, 551, 574, 575, 576, 551, 575, 576, 551, 
   574, 575, 576, 569, 570, 571, 559, 551, 662, 665, 666, 612, 613, 649, 772, 773, 
   775, 778, 779, 775, 799, 803, 808, 815, 818, 828, 833, 835, 837, 795, 821, 601, 
   675, 692, 702, 715, 730, 745, 757, 768, 787, 421, 434, 447, 460, 473, 497, 522, 
   524, 543, 589, 102, 127, 152, 177, 202, 227, 241, 278, 318, 349, 407, 56, 65, 
   70, 77, 83, 91, 24, 25, 27, 104, 105, 120, 129, 130, 145, 154, 155, 170, 
   179, 180, 195, 204, 205, 220, 243, 244, 271, 257, 259, 284, 285, 286, 286, 287, 
   288, 292, 293, 295, 320, 321, 322, 324, 325, 326, 327, 328, 329, 329, 330, 331, 
   351, 352, 353, 355, 356, 357, 362, 363, 364, 364, 365, 366, 370, 371, 373, 510, 
   512, 526, 527, 528, 529, 530, 531, 531, 532, 533, 545, 546, 547, 552, 553, 554, 
   554, 555, 556, 560, 561, 563, 622, 627, 634, 640, 648, 655, 660, 668, 670, 
};

/** Token literal values. */
public static final String[] jjstrLiteralImages = {
"", null, null, null, null, null, null, null, null, null, null, null, null, 
null, null, null, null, "\54", "\74", "\74\75", "\76", "\76\75", "\75", "\41\75", 
"\74\76", "\50", "\51", "\173", "\175", "\52", "\57", "\45", "\53", "\55", 
"\143\157\156\164\141\151\156\163", "\156\157\164\143\157\156\164\141\151\156\163", "\145\161\165\141\154\163", 
"\142\145\164\167\145\145\156", null, null, null, null, null, null, null, null, null, null, null, null, null, 
null, null, null, null, null, null, null, "\160\141\164\162\157\154\72\151\144", 
"\160\141\164\162\157\154\72\163\164\141\164\151\157\156", "\160\141\164\162\157\154\72\164\145\141\155", 
"\160\141\164\162\157\154\72\155\141\156\144\141\164\145", "\160\141\164\162\157\154\72\160\141\164\162\157\154\164\171\160\145", 
"\160\141\164\162\157\154\72\164\162\141\156\163\160\157\162\164", "\160\141\164\162\157\154\72\154\145\141\144\145\162", 
"\160\141\164\162\157\154\72\160\151\154\157\164", "\160\141\164\162\157\154\72\155\145\155\142\145\162", null, null, null, null, 
null, null, null, null, "\174", null, null, null, null, null, null, };

/** Lexer state names. */
public static final String[] lexStateNames = {
   "DEFAULT",
};
static final long[] jjtoToken = {
   0xfcfffc7fffffff81L, 0x3ffffL, 
};
static final long[] jjtoSkip = {
   0x7eL, 0x0L, 
};
static final long[] jjtoSpecial = {
   0x60L, 0x0L, 
};
protected SimpleCharStream input_stream;
private final int[] jjrounds = new int[838];
private final int[] jjstateSet = new int[1676];
protected char curChar;
/** Constructor. */
public ParserTokenManager(SimpleCharStream stream){
   if (SimpleCharStream.staticFlag)
      throw new Error("ERROR: Cannot use a static CharStream class with a non-static lexical analyzer.");
   input_stream = stream;
}

/** Constructor. */
public ParserTokenManager(SimpleCharStream stream, int lexState){
   this(stream);
   SwitchTo(lexState);
}

/** Reinitialise parser. */
public void ReInit(SimpleCharStream stream)
{
   jjmatchedPos = jjnewStateCnt = 0;
   curLexState = defaultLexState;
   input_stream = stream;
   ReInitRounds();
}
private void ReInitRounds()
{
   int i;
   jjround = 0x80000001;
   for (i = 838; i-- > 0;)
      jjrounds[i] = 0x80000000;
}

/** Reinitialise parser. */
public void ReInit(SimpleCharStream stream, int lexState)
{
   ReInit(stream);
   SwitchTo(lexState);
}

/** Switch to specified lex state. */
public void SwitchTo(int lexState)
{
   if (lexState >= 1 || lexState < 0)
      throw new TokenMgrError("Error: Ignoring invalid lexical state : " + lexState + ". State unchanged.", TokenMgrError.INVALID_LEXICAL_STATE);
   else
      curLexState = lexState;
}

protected Token jjFillToken()
{
   final Token t;
   final String curTokenImage;
   final int beginLine;
   final int endLine;
   final int beginColumn;
   final int endColumn;
   String im = jjstrLiteralImages[jjmatchedKind];
   curTokenImage = (im == null) ? input_stream.GetImage() : im;
   beginLine = input_stream.getBeginLine();
   beginColumn = input_stream.getBeginColumn();
   endLine = input_stream.getEndLine();
   endColumn = input_stream.getEndColumn();
   t = Token.newToken(jjmatchedKind, curTokenImage);

   t.beginLine = beginLine;
   t.endLine = endLine;
   t.beginColumn = beginColumn;
   t.endColumn = endColumn;

   return t;
}

int curLexState = 0;
int defaultLexState = 0;
int jjnewStateCnt;
int jjround;
int jjmatchedPos;
int jjmatchedKind;

/** Get the next Token. */
public Token getNextToken() 
{
  Token specialToken = null;
  Token matchedToken;
  int curPos = 0;

  EOFLoop :
  for (;;)
  {
   try
   {
      curChar = input_stream.BeginToken();
   }
   catch(java.io.IOException e)
   {
      jjmatchedKind = 0;
      matchedToken = jjFillToken();
      matchedToken.specialToken = specialToken;
      return matchedToken;
   }

   jjmatchedKind = 0x7fffffff;
   jjmatchedPos = 0;
   curPos = jjMoveStringLiteralDfa0_0();
   if (jjmatchedKind != 0x7fffffff)
   {
      if (jjmatchedPos + 1 < curPos)
         input_stream.backup(curPos - jjmatchedPos - 1);
      if ((jjtoToken[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L)
      {
         matchedToken = jjFillToken();
         matchedToken.specialToken = specialToken;
         return matchedToken;
      }
      else
      {
         if ((jjtoSpecial[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L)
         {
            matchedToken = jjFillToken();
            if (specialToken == null)
               specialToken = matchedToken;
            else
            {
               matchedToken.specialToken = specialToken;
               specialToken = (specialToken.next = matchedToken);
            }
         }
         continue EOFLoop;
      }
   }
   int error_line = input_stream.getEndLine();
   int error_column = input_stream.getEndColumn();
   String error_after = null;
   boolean EOFSeen = false;
   try { input_stream.readChar(); input_stream.backup(1); }
   catch (java.io.IOException e1) {
      EOFSeen = true;
      error_after = curPos <= 1 ? "" : input_stream.GetImage();
      if (curChar == '\n' || curChar == '\r') {
         error_line++;
         error_column = 0;
      }
      else
         error_column++;
   }
   if (!EOFSeen) {
      input_stream.backup(1);
      error_after = curPos <= 1 ? "" : input_stream.GetImage();
   }
   throw new TokenMgrError(EOFSeen, curLexState, error_line, error_column, error_after, curChar, TokenMgrError.LEXICAL_ERROR);
  }
}

private void jjCheckNAdd(int state)
{
   if (jjrounds[state] != jjround)
   {
      jjstateSet[jjnewStateCnt++] = state;
      jjrounds[state] = jjround;
   }
}
private void jjAddStates(int start, int end)
{
   do {
      jjstateSet[jjnewStateCnt++] = jjnextStates[start];
   } while (start++ != end);
}
private void jjCheckNAddTwoStates(int state1, int state2)
{
   jjCheckNAdd(state1);
   jjCheckNAdd(state2);
}

private void jjCheckNAddStates(int start, int end)
{
   do {
      jjCheckNAdd(jjnextStates[start]);
   } while (start++ != end);
}

}
