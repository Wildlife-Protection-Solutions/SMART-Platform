...
public class CommunityPackageConfigurator implements ICtPackageConfigurator {

	private CommunityCtPackage ctpackage;
	...
	public void createGui(Composite parent, ICtPackage ctitem, Consumer<String> validate) {
		...
		contributions.add(new CommunityConnectDataContribution().getUiController());
		...
		if (!(ctitem instanceof CommunityCtPackage)) throw new IllegalStateException("Invalid package type");
		this.ctpackage = (CommunityCtPackage) ctitem;
		...
		warnLabel.setText("WARNING: Community packages are visible to the general public. Ensure there is no sensitive data in the models used in the community packages");
		
		Label nameLabel = new Label(g, SWT.NONE);
		nameLabel.setText("Package Name:");
		
		txtName = new Text(g, SWT.BORDER);
		txtName.setText(ctitem.getName() == null ? (ctitem.getTypeIdentifier() + "Community Package") : ctitem.getName());
	...
	public Composite createDetails(Composite parent, ICtPackage ctpackage,
		...
		try(Session session = HibernateManager.openSession()){
			CommunityCtPackage local = session.get(CommunityCtPackage.class, ((CommunityCtPackage)ctpackage).getUuid());
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				try(Session session = HibernateManager.openSession()){
					...
					if (ctpackage != null && ctpackage.getUuid() != null) {
						init = session.get(CommunityCtPackage.class, ctpackage.getUuid());
