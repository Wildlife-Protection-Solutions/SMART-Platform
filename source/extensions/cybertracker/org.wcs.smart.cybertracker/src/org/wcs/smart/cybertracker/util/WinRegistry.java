...
public class WinRegistry {
    ...
    InvocationTargetException 
  {
    int rc =((Integer) regDeleteKey.invoke(root,  
        new Object[] { Long.valueOf(hkey), toCstr(key) })).intValue();
    ...
    InvocationTargetException 
  {
    ...
    for(int index=0; index<count; index++)  {
      byte[] name = (byte[]) regEnumValue.invoke(root, new Object[] {
    		  Long.valueOf
            (handles[0]), Integer.valueOf(index), Long.valueOf(maxlen + 1)});
    ...
    InvocationTargetException 
  {
    ...
    long count  = info[0]; // Fix: info[2] was being used here with wrong results. Suggested by davenpcj, confirmed by Petrucio
    long maxlen = info[3]; // value length max
    for(int index=0; index<count; index++)  {
    ...
    InvocationTargetException 
  {
    ...
    regSetValueEx.invoke(root,  
        new Object[] { 
          Long.valueOf(handles[0]), toCstr(valueName), toCstr(value) 
          }); 
    regCloseKey.invoke(root, new Object[] { Long.valueOf(handles[0]) });
  }
