...
public class PermissionTester extends PropertyTester {
	...
	public boolean test(Object receiver, String property, Object[] args, Object expectedValue) {
		...
		}else if ( expectedValue.toString().equalsIgnoreCase("create_record")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canCreateRecordAny();
		}else if ( expectedValue.toString().equalsIgnoreCase("create_entity")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canCreateEntityAny();
		}else if ( expectedValue.toString().equalsIgnoreCase("create_query")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canCreateQueryAny();
		}else if ( expectedValue.toString().equalsIgnoreCase("assessment_perspective")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canViewRecordAny();
		}else if ( expectedValue.toString().equalsIgnoreCase("analyst_perspective")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canViewEntityAny() ||
					IntelSecurityManager.INSTANCE.canViewQueryAny();
		}else if ( expectedValue.toString().equalsIgnoreCase("entity_perspective")){ //$NON-NLS-1$
			return IntelSecurityManager.INSTANCE.canViewEntityAny();
		}
		return false;
