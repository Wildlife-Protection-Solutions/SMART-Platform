...
public enum UserValidationManager {
	...
	public boolean validate(Collection<ConservationArea> toValidate, Smart6Database smart6, 
			Shell shell, SubMonitor task) {
		
			task.split(1).beginTask(Messages.ValidateUserJob_smart6subtask, 1);

			UserNamePasswordDialog udialog = new UserNamePasswordDialog(shell, Messages.ValidateUserJob_DialogTitle, Messages.ValidateUserJob_AllCaMsg, IDialogConstants.OK_LABEL);
			...
			while(!smart6error.isEmpty()) {
				...
				udialog = new UserNamePasswordDialog(shell, 
