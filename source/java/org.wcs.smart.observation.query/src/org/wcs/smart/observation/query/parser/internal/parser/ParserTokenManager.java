/* Generated By:JavaCC: Do not edit this line. ParserTokenManager.java */
package org.wcs.smart.observation.query.parser.internal.parser;
import java.util.*;
import org.wcs.smart.observation.query.parser.internal.*;
import org.wcs.smart.observation.query.model.filter.*;
import org.wcs.smart.query.model.filter.*;
import org.wcs.smart.query.model.summary.*;

/** Token Manager. */
public class ParserTokenManager implements ParserConstants
{

  /** Debug output. */
  public  java.io.PrintStream debugStream = System.out;
  /** Set debug output. */
  public  void setDebugStream(java.io.PrintStream ds) { debugStream = ds; }
private int jjStopAtPos(int pos, int kind)
{
   jjmatchedKind = kind;
   jjmatchedPos = pos;
   return pos + 1;
}
private int jjMoveStringLiteralDfa0_0()
{
   switch(curChar)
   {
      case 9:
         jjmatchedKind = 2;
         return jjMoveNfa_0(3, 0);
      case 10:
         jjmatchedKind = 4;
         return jjMoveNfa_0(3, 0);
      case 13:
         jjmatchedKind = 3;
         return jjMoveNfa_0(3, 0);
      case 32:
         jjmatchedKind = 1;
         return jjMoveNfa_0(3, 0);
      case 33:
         return jjMoveStringLiteralDfa1_0(0x10000L);
      case 40:
         jjmatchedKind = 18;
         return jjMoveNfa_0(3, 0);
      case 41:
         jjmatchedKind = 19;
         return jjMoveNfa_0(3, 0);
      case 43:
         jjmatchedKind = 21;
         return jjMoveNfa_0(3, 0);
      case 44:
         jjmatchedKind = 10;
         return jjMoveNfa_0(3, 0);
      case 45:
         jjmatchedKind = 22;
         return jjMoveNfa_0(3, 0);
      case 47:
         jjmatchedKind = 20;
         return jjMoveNfa_0(3, 0);
      case 60:
         jjmatchedKind = 11;
         return jjMoveStringLiteralDfa1_0(0x21000L);
      case 61:
         jjmatchedKind = 15;
         return jjMoveNfa_0(3, 0);
      case 62:
         jjmatchedKind = 13;
         return jjMoveStringLiteralDfa1_0(0x4000L);
      case 65:
         return jjMoveStringLiteralDfa1_0(0x80L);
      case 78:
         return jjMoveStringLiteralDfa1_0(0x100L);
      case 79:
         return jjMoveStringLiteralDfa1_0(0x200L);
      case 97:
         return jjMoveStringLiteralDfa1_0(0x80L);
      case 98:
         return jjMoveStringLiteralDfa1_0(0x4000000L);
      case 99:
         return jjMoveStringLiteralDfa1_0(0x800000L);
      case 101:
         return jjMoveStringLiteralDfa1_0(0x2000000L);
      case 110:
         return jjMoveStringLiteralDfa1_0(0x1000100L);
      case 111:
         return jjMoveStringLiteralDfa1_0(0x8000000000200L);
      case 119:
         return jjMoveStringLiteralDfa1_0(0x30000000000000L);
      case 124:
         jjmatchedKind = 61;
         return jjMoveNfa_0(3, 0);
      default :
         return jjMoveNfa_0(3, 0);
   }
}
private int jjMoveStringLiteralDfa1_0(long active0)
{
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 0);
   }
   switch(curChar)
   {
      case 61:
         if ((active0 & 0x1000L) != 0L)
         {
            jjmatchedKind = 12;
            jjmatchedPos = 1;
         }
         else if ((active0 & 0x4000L) != 0L)
         {
            jjmatchedKind = 14;
            jjmatchedPos = 1;
         }
         else if ((active0 & 0x10000L) != 0L)
         {
            jjmatchedKind = 16;
            jjmatchedPos = 1;
         }
         break;
      case 62:
         if ((active0 & 0x20000L) != 0L)
         {
            jjmatchedKind = 17;
            jjmatchedPos = 1;
         }
         break;
      case 78:
         return jjMoveStringLiteralDfa2_0(active0, 0x80L);
      case 79:
         return jjMoveStringLiteralDfa2_0(active0, 0x100L);
      case 82:
         if ((active0 & 0x200L) != 0L)
         {
            jjmatchedKind = 9;
            jjmatchedPos = 1;
         }
         break;
      case 97:
         return jjMoveStringLiteralDfa2_0(active0, 0x10000000000000L);
      case 98:
         return jjMoveStringLiteralDfa2_0(active0, 0x8000000000000L);
      case 101:
         return jjMoveStringLiteralDfa2_0(active0, 0x4000000L);
      case 110:
         return jjMoveStringLiteralDfa2_0(active0, 0x80L);
      case 111:
         return jjMoveStringLiteralDfa2_0(active0, 0x1800100L);
      case 112:
         return jjMoveStringLiteralDfa2_0(active0, 0x20000000000000L);
      case 113:
         return jjMoveStringLiteralDfa2_0(active0, 0x2000000L);
      case 114:
         if ((active0 & 0x200L) != 0L)
         {
            jjmatchedKind = 9;
            jjmatchedPos = 1;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 1);
}
private int jjMoveStringLiteralDfa2_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 1);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 1);
   }
   switch(curChar)
   {
      case 68:
         if ((active0 & 0x80L) != 0L)
         {
            jjmatchedKind = 7;
            jjmatchedPos = 2;
         }
         break;
      case 84:
         if ((active0 & 0x100L) != 0L)
         {
            jjmatchedKind = 8;
            jjmatchedPos = 2;
         }
         break;
      case 100:
         if ((active0 & 0x80L) != 0L)
         {
            jjmatchedKind = 7;
            jjmatchedPos = 2;
         }
         break;
      case 110:
         return jjMoveStringLiteralDfa3_0(active0, 0x20000000800000L);
      case 115:
         return jjMoveStringLiteralDfa3_0(active0, 0x8000000000000L);
      case 116:
         if ((active0 & 0x100L) != 0L)
         {
            jjmatchedKind = 8;
            jjmatchedPos = 2;
         }
         return jjMoveStringLiteralDfa3_0(active0, 0x5000000L);
      case 117:
         return jjMoveStringLiteralDfa3_0(active0, 0x2000000L);
      case 121:
         return jjMoveStringLiteralDfa3_0(active0, 0x10000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 2);
}
private int jjMoveStringLiteralDfa3_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 2);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 2);
   }
   switch(curChar)
   {
      case 58:
         return jjMoveStringLiteralDfa4_0(active0, 0x20000000000000L);
      case 97:
         return jjMoveStringLiteralDfa4_0(active0, 0x2000000L);
      case 99:
         return jjMoveStringLiteralDfa4_0(active0, 0x1000000L);
      case 101:
         return jjMoveStringLiteralDfa4_0(active0, 0x8000000000000L);
      case 112:
         return jjMoveStringLiteralDfa4_0(active0, 0x10000000000000L);
      case 116:
         return jjMoveStringLiteralDfa4_0(active0, 0x800000L);
      case 119:
         return jjMoveStringLiteralDfa4_0(active0, 0x4000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 3);
}
private int jjMoveStringLiteralDfa4_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 3);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 3);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa5_0(active0, 0x800000L);
      case 101:
         return jjMoveStringLiteralDfa5_0(active0, 0x4000000L);
      case 108:
         return jjMoveStringLiteralDfa5_0(active0, 0x2000000L);
      case 111:
         return jjMoveStringLiteralDfa5_0(active0, 0x10000001000000L);
      case 114:
         return jjMoveStringLiteralDfa5_0(active0, 0x8000000000000L);
      case 115:
         return jjMoveStringLiteralDfa5_0(active0, 0x20000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 4);
}
private int jjMoveStringLiteralDfa5_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 4);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 4);
   }
   switch(curChar)
   {
      case 101:
         return jjMoveStringLiteralDfa6_0(active0, 0x4000000L);
      case 105:
         return jjMoveStringLiteralDfa6_0(active0, 0x10000000800000L);
      case 110:
         return jjMoveStringLiteralDfa6_0(active0, 0x1000000L);
      case 114:
         return jjMoveStringLiteralDfa6_0(active0, 0x20000000000000L);
      case 115:
         if ((active0 & 0x2000000L) != 0L)
         {
            jjmatchedKind = 25;
            jjmatchedPos = 5;
         }
         break;
      case 118:
         return jjMoveStringLiteralDfa6_0(active0, 0x8000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 5);
}
private int jjMoveStringLiteralDfa6_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 5);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 5);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa7_0(active0, 0x8000000000000L);
      case 99:
         if ((active0 & 0x20000000000000L) != 0L)
         {
            jjmatchedKind = 53;
            jjmatchedPos = 6;
         }
         break;
      case 110:
         if ((active0 & 0x4000000L) != 0L)
         {
            jjmatchedKind = 26;
            jjmatchedPos = 6;
         }
         return jjMoveStringLiteralDfa7_0(active0, 0x10000000800000L);
      case 116:
         return jjMoveStringLiteralDfa7_0(active0, 0x1000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 6);
}
private int jjMoveStringLiteralDfa7_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 6);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 6);
   }
   switch(curChar)
   {
      case 97:
         return jjMoveStringLiteralDfa8_0(active0, 0x1000000L);
      case 115:
         if ((active0 & 0x800000L) != 0L)
         {
            jjmatchedKind = 23;
            jjmatchedPos = 7;
         }
         break;
      case 116:
         if ((active0 & 0x10000000000000L) != 0L)
         {
            jjmatchedKind = 52;
            jjmatchedPos = 7;
         }
         return jjMoveStringLiteralDfa8_0(active0, 0x8000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 7);
}
private int jjMoveStringLiteralDfa8_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 7);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 7);
   }
   switch(curChar)
   {
      case 105:
         return jjMoveStringLiteralDfa9_0(active0, 0x8000001000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 8);
}
private int jjMoveStringLiteralDfa9_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 8);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 8);
   }
   switch(curChar)
   {
      case 110:
         return jjMoveStringLiteralDfa10_0(active0, 0x1000000L);
      case 111:
         return jjMoveStringLiteralDfa10_0(active0, 0x8000000000000L);
      default :
         break;
   }
   return jjMoveNfa_0(3, 9);
}
private int jjMoveStringLiteralDfa10_0(long old0, long active0)
{
   if (((active0 &= old0)) == 0L)
      return jjMoveNfa_0(3, 9);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) {
   return jjMoveNfa_0(3, 9);
   }
   switch(curChar)
   {
      case 110:
         if ((active0 & 0x8000000000000L) != 0L)
         {
            jjmatchedKind = 51;
            jjmatchedPos = 10;
         }
         break;
      case 115:
         if ((active0 & 0x1000000L) != 0L)
         {
            jjmatchedKind = 24;
            jjmatchedPos = 10;
         }
         break;
      default :
         break;
   }
   return jjMoveNfa_0(3, 10);
}
static final long[] jjbitVec0 = {
   0x0L, 0x0L, 0xffffffffffffffffL, 0xffffffffffffffffL
};
private int jjMoveNfa_0(int startState, int curPos)
{
   int strKind = jjmatchedKind;
   int strPos = jjmatchedPos;
   int seenUpto;
   input_stream.backup(seenUpto = curPos + 1);
   try { curChar = input_stream.readChar(); }
   catch(java.io.IOException e) { throw new Error("Internal Error"); }
   curPos = 0;
   int startsAt = 0;
   jjnewStateCnt = 759;
   int i = 1;
   jjstateSet[0] = startState;
   int kind = 0x7fffffff;
   for (;;)
   {
      if (++jjround == 0x7fffffff)
         ReInitRounds();
      if (curChar < 64)
      {
         long l = 1L << curChar;
         do
         {
            switch(jjstateSet[--i])
            {
               case 3:
                  if ((0x3ff000000000000L & l) != 0L)
                  {
                     if (kind > 27)
                        kind = 27;
                     jjCheckNAddStates(0, 7);
                  }
                  else if ((0x280000000000L & l) != 0L)
                     jjAddStates(8, 11);
                  else if (curChar == 47)
                     jjAddStates(12, 13);
                  else if (curChar == 46)
                     jjCheckNAddTwoStates(86, 88);
                  else if (curChar == 34)
                     jjCheckNAddStates(14, 16);
                  break;
               case 13:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjstateSet[jjnewStateCnt++] = 13;
                  break;
               case 34:
                  if (curChar == 58)
                     jjAddStates(17, 21);
                  break;
               case 61:
                  if (curChar != 58)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddTwoStates(62, 64);
                  break;
               case 63:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddTwoStates(63, 64);
                  break;
               case 64:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 65;
                  break;
               case 66:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddTwoStates(64, 66);
                  break;
               case 70:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 69;
                  break;
               case 74:
                  if (curChar == 34)
                     jjCheckNAddStates(14, 16);
                  break;
               case 75:
                  if ((0xfffffffbffffdbffL & l) != 0L)
                     jjCheckNAddStates(14, 16);
                  break;
               case 77:
                  if ((0xffffffffffffdbffL & l) != 0L)
                     jjCheckNAddStates(14, 16);
                  break;
               case 78:
                  if (curChar == 34 && kind > 67)
                     kind = 67;
                  break;
               case 84:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(8, 11);
                  break;
               case 85:
                  if (curChar == 46)
                     jjCheckNAdd(86);
                  break;
               case 86:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(86);
                  break;
               case 87:
                  if (curChar == 46)
                     jjCheckNAdd(88);
                  break;
               case 88:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddTwoStates(88, 89);
                  break;
               case 90:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(22, 23);
                  break;
               case 91:
                  if (curChar == 46)
                     jjCheckNAdd(92);
                  break;
               case 92:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(92);
                  break;
               case 93:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddStates(24, 26);
                  break;
               case 94:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(94);
                  break;
               case 95:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddTwoStates(95, 96);
                  break;
               case 96:
                  if (curChar == 46)
                     jjCheckNAdd(97);
                  break;
               case 97:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(97);
                  break;
               case 98:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddStates(27, 30);
                  break;
               case 99:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddTwoStates(99, 89);
                  break;
               case 100:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddStates(31, 33);
                  break;
               case 101:
                  if (curChar == 46)
                     jjCheckNAdd(102);
                  break;
               case 102:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddTwoStates(102, 89);
                  break;
               case 103:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddStates(34, 36);
                  break;
               case 104:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(104);
                  break;
               case 105:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddTwoStates(105, 106);
                  break;
               case 106:
                  if (curChar == 46)
                     jjCheckNAdd(107);
                  break;
               case 107:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAdd(107);
                  break;
               case 108:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(86, 88);
                  break;
               case 109:
                  if (curChar == 47)
                     jjAddStates(12, 13);
                  break;
               case 110:
                  if (curChar != 47)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjCheckNAdd(111);
                  break;
               case 111:
                  if ((0xffffffffffffdbffL & l) == 0L)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjCheckNAdd(111);
                  break;
               case 112:
                  if (curChar == 42)
                     jjCheckNAddTwoStates(113, 114);
                  break;
               case 113:
                  if ((0xfffffbffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(113, 114);
                  break;
               case 114:
                  if (curChar == 42)
                     jjCheckNAddStates(37, 39);
                  break;
               case 115:
                  if ((0xffff7bffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(116, 114);
                  break;
               case 116:
                  if ((0xfffffbffffffffffL & l) != 0L)
                     jjCheckNAddTwoStates(116, 114);
                  break;
               case 117:
                  if (curChar == 47 && kind > 6)
                     kind = 6;
                  break;
               case 119:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 120;
                  break;
               case 121:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 32)
                     kind = 32;
                  jjstateSet[jjnewStateCnt++] = 121;
                  break;
               case 129:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 130;
                  break;
               case 131:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(40, 41);
                  break;
               case 132:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 133;
                  break;
               case 134:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 39)
                     kind = 39;
                  jjstateSet[jjnewStateCnt++] = 134;
                  break;
               case 136:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 135;
                  break;
               case 146:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 145;
                  break;
               case 154:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 155;
                  break;
               case 156:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(42, 43);
                  break;
               case 157:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 158;
                  break;
               case 159:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 40)
                     kind = 40;
                  jjstateSet[jjnewStateCnt++] = 159;
                  break;
               case 161:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 160;
                  break;
               case 171:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 170;
                  break;
               case 179:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 180;
                  break;
               case 181:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(44, 45);
                  break;
               case 182:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 183;
                  break;
               case 184:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 41)
                     kind = 41;
                  jjstateSet[jjnewStateCnt++] = 184;
                  break;
               case 186:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 185;
                  break;
               case 196:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 195;
                  break;
               case 204:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 205;
                  break;
               case 206:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(46, 47);
                  break;
               case 207:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 208;
                  break;
               case 209:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 42)
                     kind = 42;
                  jjstateSet[jjnewStateCnt++] = 209;
                  break;
               case 211:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 210;
                  break;
               case 221:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 220;
                  break;
               case 229:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 230;
                  break;
               case 231:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(48, 49);
                  break;
               case 232:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 233;
                  break;
               case 234:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjstateSet[jjnewStateCnt++] = 234;
                  break;
               case 236:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 235;
                  break;
               case 246:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 245;
                  break;
               case 254:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 255;
                  break;
               case 256:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(50, 51);
                  break;
               case 257:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 258;
                  break;
               case 259:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjstateSet[jjnewStateCnt++] = 259;
                  break;
               case 261:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 260;
                  break;
               case 271:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 270;
                  break;
               case 279:
                  if (curChar == 58)
                     jjAddStates(52, 53);
                  break;
               case 281:
                  if (curChar != 58)
                     break;
                  if (kind > 54)
                     kind = 54;
                  jjstateSet[jjnewStateCnt++] = 282;
                  break;
               case 283:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 54)
                     kind = 54;
                  jjstateSet[jjnewStateCnt++] = 283;
                  break;
               case 291:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 290;
                  break;
               case 299:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 300;
                  break;
               case 301:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(54, 55);
                  break;
               case 302:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 328;
                  break;
               case 303:
                  if (curChar == 58)
                     jjAddStates(56, 58);
                  break;
               case 305:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 306;
                  break;
               case 307:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 56)
                     kind = 56;
                  jjstateSet[jjnewStateCnt++] = 307;
                  break;
               case 319:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 318;
                  break;
               case 336:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 337;
                  break;
               case 338:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(59, 60);
                  break;
               case 339:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 363;
                  break;
               case 340:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 341;
                  break;
               case 342:
                  if (curChar == 58)
                     jjAddStates(61, 62);
                  break;
               case 344:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 345;
                  break;
               case 346:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 58)
                     kind = 58;
                  jjstateSet[jjnewStateCnt++] = 346;
                  break;
               case 354:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 353;
                  break;
               case 371:
                  if (curChar == 58)
                     jjAddStates(63, 65);
                  break;
               case 372:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(66, 69);
                  break;
               case 373:
                  if (curChar == 46)
                     jjCheckNAdd(374);
                  break;
               case 374:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(374, 375);
                  break;
               case 375:
                  if (curChar != 58)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddTwoStates(376, 378);
                  break;
               case 377:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddTwoStates(377, 378);
                  break;
               case 378:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 379;
                  break;
               case 380:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddTwoStates(378, 380);
                  break;
               case 381:
                  if (curChar == 46)
                     jjCheckNAdd(382);
                  break;
               case 382:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(70, 72);
                  break;
               case 384:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(73, 74);
                  break;
               case 385:
                  if (curChar == 46)
                     jjCheckNAdd(386);
                  break;
               case 386:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(386, 375);
                  break;
               case 387:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(75, 78);
                  break;
               case 388:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(388, 375);
                  break;
               case 389:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(79, 81);
                  break;
               case 390:
                  if (curChar == 46)
                     jjCheckNAdd(391);
                  break;
               case 391:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(391, 375);
                  break;
               case 392:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(82, 86);
                  break;
               case 393:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(87, 89);
                  break;
               case 394:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(90, 93);
                  break;
               case 395:
                  if (curChar == 46)
                     jjCheckNAdd(396);
                  break;
               case 396:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(94, 96);
                  break;
               case 397:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(97, 100);
                  break;
               case 398:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(398, 375);
                  break;
               case 399:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(101, 103);
                  break;
               case 400:
                  if (curChar == 46)
                     jjCheckNAdd(401);
                  break;
               case 401:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(401, 375);
                  break;
               case 402:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(374, 382);
                  break;
               case 403:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(104, 111);
                  break;
               case 411:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 412;
                  break;
               case 413:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(112, 113);
                  break;
               case 414:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 434;
                  break;
               case 415:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 416;
                  break;
               case 417:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(114, 115);
                  break;
               case 418:
                  if (curChar != 58)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(419, 421);
                  break;
               case 420:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(420, 421);
                  break;
               case 421:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 422;
                  break;
               case 423:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(421, 423);
                  break;
               case 425:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 424;
                  break;
               case 442:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 443;
                  break;
               case 444:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(116, 117);
                  break;
               case 445:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 492;
                  break;
               case 446:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 447;
                  break;
               case 448:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(118, 119);
                  break;
               case 449:
                  if (curChar == 58)
                     jjAddStates(120, 122);
                  break;
               case 450:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(123, 126);
                  break;
               case 451:
                  if (curChar == 46)
                     jjCheckNAdd(452);
                  break;
               case 452:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(452, 453);
                  break;
               case 453:
                  if (curChar != 58)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(454, 456);
                  break;
               case 455:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(455, 456);
                  break;
               case 456:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 457;
                  break;
               case 458:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(456, 458);
                  break;
               case 459:
                  if (curChar == 46)
                     jjCheckNAdd(460);
                  break;
               case 460:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(127, 129);
                  break;
               case 462:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(130, 131);
                  break;
               case 463:
                  if (curChar == 46)
                     jjCheckNAdd(464);
                  break;
               case 464:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(464, 453);
                  break;
               case 465:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(132, 135);
                  break;
               case 466:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(466, 453);
                  break;
               case 467:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(136, 138);
                  break;
               case 468:
                  if (curChar == 46)
                     jjCheckNAdd(469);
                  break;
               case 469:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(469, 453);
                  break;
               case 470:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(139, 143);
                  break;
               case 471:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(144, 146);
                  break;
               case 472:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(147, 150);
                  break;
               case 473:
                  if (curChar == 46)
                     jjCheckNAdd(474);
                  break;
               case 474:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(151, 153);
                  break;
               case 475:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(154, 157);
                  break;
               case 476:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(476, 453);
                  break;
               case 477:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(158, 160);
                  break;
               case 478:
                  if (curChar == 46)
                     jjCheckNAdd(479);
                  break;
               case 479:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(479, 453);
                  break;
               case 480:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(452, 460);
                  break;
               case 481:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(161, 168);
                  break;
               case 483:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 482;
                  break;
               case 501:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 502;
                  break;
               case 503:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 33)
                     kind = 33;
                  jjstateSet[jjnewStateCnt++] = 503;
                  break;
               case 505:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 504;
                  break;
               case 514:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 515;
                  break;
               case 516:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 34)
                     kind = 34;
                  jjstateSet[jjnewStateCnt++] = 516;
                  break;
               case 518:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 517;
                  break;
               case 527:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 528;
                  break;
               case 529:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 35)
                     kind = 35;
                  jjstateSet[jjnewStateCnt++] = 529;
                  break;
               case 531:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 530;
                  break;
               case 540:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 541;
                  break;
               case 542:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 36)
                     kind = 36;
                  jjstateSet[jjnewStateCnt++] = 542;
                  break;
               case 544:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 543;
                  break;
               case 553:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 554;
                  break;
               case 555:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 37)
                     kind = 37;
                  jjstateSet[jjnewStateCnt++] = 555;
                  break;
               case 557:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 556;
                  break;
               case 566:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 567;
                  break;
               case 568:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjstateSet[jjnewStateCnt++] = 568;
                  break;
               case 570:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 569;
                  break;
               case 579:
                  if (curChar == 58)
                     jjAddStates(169, 170);
                  break;
               case 581:
                  if (curChar == 58)
                     jjAddStates(171, 175);
                  break;
               case 583:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 584;
                  break;
               case 585:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjstateSet[jjnewStateCnt++] = 585;
                  break;
               case 607:
                  if (curChar == 58)
                     jjAddStates(176, 178);
                  break;
               case 609:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 610;
                  break;
               case 611:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjstateSet[jjnewStateCnt++] = 611;
                  break;
               case 623:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 622;
                  break;
               case 632:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 633;
                  break;
               case 634:
                  if (curChar == 58)
                     jjAddStates(179, 180);
                  break;
               case 636:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 637;
                  break;
               case 638:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 57)
                     kind = 57;
                  jjstateSet[jjnewStateCnt++] = 638;
                  break;
               case 646:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 645;
                  break;
               case 657:
                  if (curChar == 58)
                     jjAddStates(181, 185);
                  break;
               case 659:
                  if (curChar != 58)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddTwoStates(660, 662);
                  break;
               case 661:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddTwoStates(661, 662);
                  break;
               case 662:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 663;
                  break;
               case 664:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddTwoStates(662, 664);
                  break;
               case 684:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 685;
                  break;
               case 686:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(186, 187);
                  break;
               case 687:
                  if (curChar != 58)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(688, 690);
                  break;
               case 689:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(689, 690);
                  break;
               case 690:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 691;
                  break;
               case 692:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(690, 692);
                  break;
               case 694:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 693;
                  break;
               case 703:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 704;
                  break;
               case 705:
                  if ((0x3ff400000000000L & l) != 0L)
                     jjAddStates(188, 189);
                  break;
               case 706:
                  if (curChar == 58)
                     jjAddStates(190, 192);
                  break;
               case 707:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(193, 196);
                  break;
               case 708:
                  if (curChar == 46)
                     jjCheckNAdd(709);
                  break;
               case 709:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(709, 710);
                  break;
               case 710:
                  if (curChar != 58)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(711, 713);
                  break;
               case 712:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(712, 713);
                  break;
               case 713:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 714;
                  break;
               case 715:
                  if ((0x3ff400000000000L & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(713, 715);
                  break;
               case 716:
                  if (curChar == 46)
                     jjCheckNAdd(717);
                  break;
               case 717:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(197, 199);
                  break;
               case 719:
                  if ((0x280000000000L & l) != 0L)
                     jjAddStates(200, 201);
                  break;
               case 720:
                  if (curChar == 46)
                     jjCheckNAdd(721);
                  break;
               case 721:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(721, 710);
                  break;
               case 722:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(202, 205);
                  break;
               case 723:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(723, 710);
                  break;
               case 724:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(206, 208);
                  break;
               case 725:
                  if (curChar == 46)
                     jjCheckNAdd(726);
                  break;
               case 726:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(726, 710);
                  break;
               case 727:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(209, 213);
                  break;
               case 728:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(214, 216);
                  break;
               case 729:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(217, 220);
                  break;
               case 730:
                  if (curChar == 46)
                     jjCheckNAdd(731);
                  break;
               case 731:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(221, 223);
                  break;
               case 732:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(224, 227);
                  break;
               case 733:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(733, 710);
                  break;
               case 734:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(228, 230);
                  break;
               case 735:
                  if (curChar == 46)
                     jjCheckNAdd(736);
                  break;
               case 736:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddTwoStates(736, 710);
                  break;
               case 737:
                  if (curChar == 46)
                     jjCheckNAddTwoStates(709, 717);
                  break;
               case 738:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjCheckNAddStates(231, 238);
                  break;
               case 740:
                  if (curChar == 58)
                     jjstateSet[jjnewStateCnt++] = 739;
                  break;
               case 749:
                  if ((0x3ff000000000000L & l) == 0L)
                     break;
                  if (kind > 27)
                     kind = 27;
                  jjCheckNAddStates(0, 7);
                  break;
               case 750:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 751;
                  break;
               case 751:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 752;
                  break;
               case 752:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 753;
                  break;
               case 753:
                  if (curChar == 45)
                     jjstateSet[jjnewStateCnt++] = 754;
                  break;
               case 754:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 755;
                  break;
               case 755:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 756;
                  break;
               case 756:
                  if (curChar == 45)
                     jjstateSet[jjnewStateCnt++] = 757;
                  break;
               case 757:
                  if ((0x3ff000000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 758;
                  break;
               case 758:
                  if ((0x3ff000000000000L & l) != 0L && kind > 46)
                     kind = 46;
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      else if (curChar < 128)
      {
         long l = 1L << (curChar & 077);
         do
         {
            switch(jjstateSet[--i])
            {
               case 3:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                  {
                     if (kind > 45)
                        kind = 45;
                     jjCheckNAddTwoStates(12, 13);
                  }
                  if (curChar == 97)
                     jjAddStates(239, 251);
                  else if (curChar == 99)
                     jjAddStates(252, 264);
                  else if (curChar == 109)
                     jjAddStates(265, 266);
                  else if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 72;
                  else if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 59;
                  else if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 32;
                  else if (curChar == 80)
                     jjstateSet[jjnewStateCnt++] = 29;
                  else if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 24;
                  else if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 20;
                  else if (curChar == 66)
                     jjCheckNAdd(14);
                  else if (curChar == 67)
                     jjCheckNAdd(14);
                  else if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 10;
                  else if (curChar == 102)
                     jjstateSet[jjnewStateCnt++] = 6;
                  else if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 2;
                  break;
               case 0:
                  if (curChar == 101 && kind > 31)
                     kind = 31;
                  break;
               case 1:
                  if (curChar == 117)
                     jjCheckNAdd(0);
                  break;
               case 2:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 1;
                  break;
               case 4:
                  if (curChar == 115)
                     jjCheckNAdd(0);
                  break;
               case 5:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 4;
                  break;
               case 6:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 5;
                  break;
               case 7:
                  if (curChar == 102)
                     jjstateSet[jjnewStateCnt++] = 6;
                  break;
               case 8:
                  if (curChar == 108 && kind > 31)
                     kind = 31;
                  break;
               case 9:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 8;
                  break;
               case 10:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 9;
                  break;
               case 11:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 10;
                  break;
               case 12:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjCheckNAddTwoStates(12, 13);
                  break;
               case 13:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 45)
                     kind = 45;
                  jjCheckNAdd(13);
                  break;
               case 14:
                  if (curChar == 65 && kind > 49)
                     kind = 49;
                  break;
               case 15:
                  if (curChar == 67)
                     jjCheckNAdd(14);
                  break;
               case 16:
                  if (curChar == 66)
                     jjCheckNAdd(14);
                  break;
               case 17:
                  if (curChar == 78 && kind > 49)
                     kind = 49;
                  break;
               case 18:
                  if (curChar == 73)
                     jjstateSet[jjnewStateCnt++] = 17;
                  break;
               case 19:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 18;
                  break;
               case 20:
                  if (curChar == 68)
                     jjstateSet[jjnewStateCnt++] = 19;
                  break;
               case 21:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 20;
                  break;
               case 22:
                  if (curChar == 84 && kind > 49)
                     kind = 49;
                  break;
               case 23:
                  if (curChar == 71)
                     jjstateSet[jjnewStateCnt++] = 22;
                  break;
               case 24:
                  if (curChar == 78)
                     jjstateSet[jjnewStateCnt++] = 23;
                  break;
               case 25:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 24;
                  break;
               case 26:
                  if (curChar == 76 && kind > 49)
                     kind = 49;
                  break;
               case 27:
                  if (curChar == 82)
                     jjstateSet[jjnewStateCnt++] = 26;
                  break;
               case 28:
                  if (curChar == 84)
                     jjstateSet[jjnewStateCnt++] = 27;
                  break;
               case 29:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 28;
                  break;
               case 30:
                  if (curChar == 80)
                     jjstateSet[jjnewStateCnt++] = 29;
                  break;
               case 31:
                  if (curChar == 109 && kind > 59)
                     kind = 59;
                  break;
               case 32:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 31;
                  break;
               case 33:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 32;
                  break;
               case 35:
                  if (curChar == 121 && kind > 60)
                     kind = 60;
                  break;
               case 36:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 35;
                  break;
               case 37:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 36;
                  break;
               case 38:
                  if (curChar == 107 && kind > 60)
                     kind = 60;
                  break;
               case 39:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 38;
                  break;
               case 40:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 39;
                  break;
               case 41:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 40;
                  break;
               case 42:
                  if (curChar == 104 && kind > 60)
                     kind = 60;
                  break;
               case 43:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 42;
                  break;
               case 44:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 43;
                  break;
               case 45:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 44;
                  break;
               case 46:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 45;
                  break;
               case 47:
                  if (curChar == 114 && kind > 60)
                     kind = 60;
                  break;
               case 48:
                  if (curChar == 101)
                     jjCheckNAdd(47);
                  break;
               case 49:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 48;
                  break;
               case 50:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 49;
                  break;
               case 51:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 50;
                  break;
               case 52:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 51;
                  break;
               case 53:
                  if (curChar == 113)
                     jjstateSet[jjnewStateCnt++] = 52;
                  break;
               case 54:
                  if (curChar == 97)
                     jjCheckNAdd(47);
                  break;
               case 55:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 54;
                  break;
               case 56:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 55;
                  break;
               case 57:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 34;
                  break;
               case 58:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 57;
                  break;
               case 59:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 58;
                  break;
               case 60:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 59;
                  break;
               case 62:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddStates(267, 269);
                  break;
               case 63:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddTwoStates(63, 64);
                  break;
               case 65:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddStates(270, 272);
                  break;
               case 66:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 66)
                     kind = 66;
                  jjCheckNAddTwoStates(64, 66);
                  break;
               case 67:
                  if (curChar == 99)
                     jjstateSet[jjnewStateCnt++] = 61;
                  break;
               case 68:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 67;
                  break;
               case 69:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 68;
                  break;
               case 71:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 70;
                  break;
               case 72:
                  if (curChar == 112)
                     jjstateSet[jjnewStateCnt++] = 71;
                  break;
               case 73:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 72;
                  break;
               case 75:
                  if ((0xffffffffefffffffL & l) != 0L)
                     jjCheckNAddStates(14, 16);
                  break;
               case 76:
                  if (curChar == 92)
                     jjstateSet[jjnewStateCnt++] = 77;
                  break;
               case 77:
                  jjCheckNAddStates(14, 16);
                  break;
               case 79:
                  if (curChar == 109)
                     jjAddStates(265, 266);
                  break;
               case 80:
                  if (curChar == 110 && kind > 59)
                     kind = 59;
                  break;
               case 81:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 80;
                  break;
               case 82:
                  if (curChar == 120 && kind > 59)
                     kind = 59;
                  break;
               case 83:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 82;
                  break;
               case 89:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(273, 275);
                  break;
               case 111:
                  if (kind > 5)
                     kind = 5;
                  jjstateSet[jjnewStateCnt++] = 111;
                  break;
               case 113:
                  jjCheckNAddTwoStates(113, 114);
                  break;
               case 115:
               case 116:
                  jjCheckNAddTwoStates(116, 114);
                  break;
               case 118:
                  if (curChar == 99)
                     jjAddStates(252, 264);
                  break;
               case 120:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 32)
                     kind = 32;
                  jjCheckNAddTwoStates(120, 121);
                  break;
               case 121:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 32)
                     kind = 32;
                  jjCheckNAdd(121);
                  break;
               case 122:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 119;
                  break;
               case 123:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 122;
                  break;
               case 124:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 123;
                  break;
               case 125:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 124;
                  break;
               case 126:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 125;
                  break;
               case 127:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 126;
                  break;
               case 128:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 127;
                  break;
               case 130:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(276, 278);
                  break;
               case 131:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(131, 146);
                  break;
               case 133:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 39)
                     kind = 39;
                  jjCheckNAddTwoStates(133, 134);
                  break;
               case 134:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 39)
                     kind = 39;
                  jjCheckNAdd(134);
                  break;
               case 135:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 132;
                  break;
               case 137:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 136;
                  break;
               case 138:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 137;
                  break;
               case 139:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 138;
                  break;
               case 140:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 139;
                  break;
               case 141:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 140;
                  break;
               case 142:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 141;
                  break;
               case 143:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 142;
                  break;
               case 144:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 143;
                  break;
               case 145:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 144;
                  break;
               case 147:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 129;
                  break;
               case 148:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 147;
                  break;
               case 149:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 148;
                  break;
               case 150:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 149;
                  break;
               case 151:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 150;
                  break;
               case 152:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 151;
                  break;
               case 153:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 152;
                  break;
               case 155:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(279, 281);
                  break;
               case 156:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(156, 171);
                  break;
               case 158:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 40)
                     kind = 40;
                  jjCheckNAddTwoStates(158, 159);
                  break;
               case 159:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 40)
                     kind = 40;
                  jjCheckNAdd(159);
                  break;
               case 160:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 157;
                  break;
               case 162:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 161;
                  break;
               case 163:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 162;
                  break;
               case 164:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 163;
                  break;
               case 165:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 164;
                  break;
               case 166:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 165;
                  break;
               case 167:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 166;
                  break;
               case 168:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 167;
                  break;
               case 169:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 168;
                  break;
               case 170:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 169;
                  break;
               case 172:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 154;
                  break;
               case 173:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 172;
                  break;
               case 174:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 173;
                  break;
               case 175:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 174;
                  break;
               case 176:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 175;
                  break;
               case 177:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 176;
                  break;
               case 178:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 177;
                  break;
               case 180:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(282, 284);
                  break;
               case 181:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(181, 196);
                  break;
               case 183:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 41)
                     kind = 41;
                  jjCheckNAddTwoStates(183, 184);
                  break;
               case 184:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 41)
                     kind = 41;
                  jjCheckNAdd(184);
                  break;
               case 185:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 182;
                  break;
               case 187:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 186;
                  break;
               case 188:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 187;
                  break;
               case 189:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 188;
                  break;
               case 190:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 189;
                  break;
               case 191:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 190;
                  break;
               case 192:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 191;
                  break;
               case 193:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 192;
                  break;
               case 194:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 193;
                  break;
               case 195:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 194;
                  break;
               case 197:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 179;
                  break;
               case 198:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 197;
                  break;
               case 199:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 198;
                  break;
               case 200:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 199;
                  break;
               case 201:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 200;
                  break;
               case 202:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 201;
                  break;
               case 203:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 202;
                  break;
               case 205:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(285, 287);
                  break;
               case 206:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(206, 221);
                  break;
               case 208:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 42)
                     kind = 42;
                  jjCheckNAddTwoStates(208, 209);
                  break;
               case 209:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 42)
                     kind = 42;
                  jjCheckNAdd(209);
                  break;
               case 210:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 207;
                  break;
               case 212:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 211;
                  break;
               case 213:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 212;
                  break;
               case 214:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 213;
                  break;
               case 215:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 214;
                  break;
               case 216:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 215;
                  break;
               case 217:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 216;
                  break;
               case 218:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 217;
                  break;
               case 219:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 218;
                  break;
               case 220:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 219;
                  break;
               case 222:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 204;
                  break;
               case 223:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 222;
                  break;
               case 224:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 223;
                  break;
               case 225:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 224;
                  break;
               case 226:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 225;
                  break;
               case 227:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 226;
                  break;
               case 228:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 227;
                  break;
               case 230:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(288, 290);
                  break;
               case 231:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(231, 246);
                  break;
               case 233:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjCheckNAddTwoStates(233, 234);
                  break;
               case 234:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 43)
                     kind = 43;
                  jjCheckNAdd(234);
                  break;
               case 235:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 232;
                  break;
               case 237:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 236;
                  break;
               case 238:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 237;
                  break;
               case 239:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 238;
                  break;
               case 240:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 239;
                  break;
               case 241:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 240;
                  break;
               case 242:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 241;
                  break;
               case 243:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 242;
                  break;
               case 244:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 243;
                  break;
               case 245:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 244;
                  break;
               case 247:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 229;
                  break;
               case 248:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 247;
                  break;
               case 249:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 248;
                  break;
               case 250:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 249;
                  break;
               case 251:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 250;
                  break;
               case 252:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 251;
                  break;
               case 253:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 252;
                  break;
               case 255:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(291, 293);
                  break;
               case 256:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(256, 271);
                  break;
               case 258:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjCheckNAddTwoStates(258, 259);
                  break;
               case 259:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 44)
                     kind = 44;
                  jjCheckNAdd(259);
                  break;
               case 260:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 257;
                  break;
               case 262:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 261;
                  break;
               case 263:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 262;
                  break;
               case 264:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 263;
                  break;
               case 265:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 264;
                  break;
               case 266:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 265;
                  break;
               case 267:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 266;
                  break;
               case 268:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 267;
                  break;
               case 269:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 268;
                  break;
               case 270:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 269;
                  break;
               case 272:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 254;
                  break;
               case 273:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 272;
                  break;
               case 274:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 273;
                  break;
               case 275:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 274;
                  break;
               case 276:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 275;
                  break;
               case 277:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 276;
                  break;
               case 278:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 277;
                  break;
               case 280:
                  if (curChar == 115)
                     jjCheckNAdd(281);
                  break;
               case 282:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 54)
                     kind = 54;
                  jjCheckNAddTwoStates(282, 283);
                  break;
               case 283:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 54)
                     kind = 54;
                  jjCheckNAdd(283);
                  break;
               case 284:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 280;
                  break;
               case 285:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 284;
                  break;
               case 286:
                  if (curChar == 112)
                     jjCheckNAdd(281);
                  break;
               case 287:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 286;
                  break;
               case 288:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 279;
                  break;
               case 289:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 288;
                  break;
               case 290:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 289;
                  break;
               case 292:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 291;
                  break;
               case 293:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 292;
                  break;
               case 294:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 293;
                  break;
               case 295:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 294;
                  break;
               case 296:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 295;
                  break;
               case 297:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 296;
                  break;
               case 298:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 297;
                  break;
               case 300:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(294, 296);
                  break;
               case 301:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(301, 302);
                  break;
               case 304:
                  if (curChar == 109)
                     jjCheckNAdd(305);
                  break;
               case 306:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 56)
                     kind = 56;
                  jjCheckNAddTwoStates(306, 307);
                  break;
               case 307:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 56)
                     kind = 56;
                  jjCheckNAdd(307);
                  break;
               case 308:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 304;
                  break;
               case 309:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 308;
                  break;
               case 310:
                  if (curChar == 103)
                     jjCheckNAdd(305);
                  break;
               case 311:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 310;
                  break;
               case 312:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 311;
                  break;
               case 313:
                  if (curChar == 109)
                     jjAddStates(297, 298);
                  break;
               case 314:
                  if (curChar == 110)
                     jjCheckNAdd(305);
                  break;
               case 315:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 314;
                  break;
               case 316:
                  if (curChar == 120)
                     jjCheckNAdd(305);
                  break;
               case 317:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 316;
                  break;
               case 318:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 303;
                  break;
               case 320:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 319;
                  break;
               case 321:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 320;
                  break;
               case 322:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 321;
                  break;
               case 323:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 322;
                  break;
               case 324:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 323;
                  break;
               case 325:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 324;
                  break;
               case 326:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 325;
                  break;
               case 327:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 326;
                  break;
               case 328:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 327;
                  break;
               case 329:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 299;
                  break;
               case 330:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 329;
                  break;
               case 331:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 330;
                  break;
               case 332:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 331;
                  break;
               case 333:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 332;
                  break;
               case 334:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 333;
                  break;
               case 335:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 334;
                  break;
               case 337:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(299, 301);
                  break;
               case 338:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(338, 339);
                  break;
               case 341:
                  if ((0x10100000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 354;
                  break;
               case 343:
                  if (curChar == 115)
                     jjCheckNAdd(344);
                  break;
               case 345:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 58)
                     kind = 58;
                  jjCheckNAddTwoStates(345, 346);
                  break;
               case 346:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 58)
                     kind = 58;
                  jjCheckNAdd(346);
                  break;
               case 347:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 343;
                  break;
               case 348:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 347;
                  break;
               case 349:
                  if (curChar == 112)
                     jjCheckNAdd(344);
                  break;
               case 350:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 349;
                  break;
               case 351:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 342;
                  break;
               case 352:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 351;
                  break;
               case 353:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 352;
                  break;
               case 355:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 340;
                  break;
               case 356:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 355;
                  break;
               case 357:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 356;
                  break;
               case 358:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 357;
                  break;
               case 359:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 358;
                  break;
               case 360:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 359;
                  break;
               case 361:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 360;
                  break;
               case 362:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 361;
                  break;
               case 363:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 362;
                  break;
               case 364:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 336;
                  break;
               case 365:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 364;
                  break;
               case 366:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 365;
                  break;
               case 367:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 366;
                  break;
               case 368:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 367;
                  break;
               case 369:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 368;
                  break;
               case 370:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 369;
                  break;
               case 376:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddStates(302, 304);
                  break;
               case 377:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddTwoStates(377, 378);
                  break;
               case 379:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddStates(305, 307);
                  break;
               case 380:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 62)
                     kind = 62;
                  jjCheckNAddTwoStates(378, 380);
                  break;
               case 383:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(308, 310);
                  break;
               case 404:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 371;
                  break;
               case 405:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 404;
                  break;
               case 406:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 405;
                  break;
               case 407:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 406;
                  break;
               case 408:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 407;
                  break;
               case 409:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 408;
                  break;
               case 410:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 409;
                  break;
               case 412:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(311, 313);
                  break;
               case 413:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(413, 414);
                  break;
               case 416:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(314, 316);
                  break;
               case 417:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(417, 418);
                  break;
               case 419:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddStates(317, 319);
                  break;
               case 420:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(420, 421);
                  break;
               case 422:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddStates(320, 322);
                  break;
               case 423:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(421, 423);
                  break;
               case 424:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 415;
                  break;
               case 426:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 425;
                  break;
               case 427:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 426;
                  break;
               case 428:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 427;
                  break;
               case 429:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 428;
                  break;
               case 430:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 429;
                  break;
               case 431:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 430;
                  break;
               case 432:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 431;
                  break;
               case 433:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 432;
                  break;
               case 434:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 433;
                  break;
               case 435:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 411;
                  break;
               case 436:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 435;
                  break;
               case 437:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 436;
                  break;
               case 438:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 437;
                  break;
               case 439:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 438;
                  break;
               case 440:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 439;
                  break;
               case 441:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 440;
                  break;
               case 443:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(323, 325);
                  break;
               case 444:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(444, 445);
                  break;
               case 447:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(326, 328);
                  break;
               case 448:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(448, 449);
                  break;
               case 454:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddStates(329, 331);
                  break;
               case 455:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(455, 456);
                  break;
               case 457:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddStates(332, 334);
                  break;
               case 458:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 65)
                     kind = 65;
                  jjCheckNAddTwoStates(456, 458);
                  break;
               case 461:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(335, 337);
                  break;
               case 482:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 446;
                  break;
               case 484:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 483;
                  break;
               case 485:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 484;
                  break;
               case 486:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 485;
                  break;
               case 487:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 486;
                  break;
               case 488:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 487;
                  break;
               case 489:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 488;
                  break;
               case 490:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 489;
                  break;
               case 491:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 490;
                  break;
               case 492:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 491;
                  break;
               case 493:
                  if (curChar == 121)
                     jjstateSet[jjnewStateCnt++] = 442;
                  break;
               case 494:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 493;
                  break;
               case 495:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 494;
                  break;
               case 496:
                  if (curChar == 103)
                     jjstateSet[jjnewStateCnt++] = 495;
                  break;
               case 497:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 496;
                  break;
               case 498:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 497;
                  break;
               case 499:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 498;
                  break;
               case 500:
                  if (curChar == 97)
                     jjAddStates(239, 251);
                  break;
               case 502:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 33)
                     kind = 33;
                  jjCheckNAddTwoStates(502, 503);
                  break;
               case 503:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 33)
                     kind = 33;
                  jjCheckNAdd(503);
                  break;
               case 504:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 501;
                  break;
               case 506:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 505;
                  break;
               case 507:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 506;
                  break;
               case 508:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 507;
                  break;
               case 509:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 508;
                  break;
               case 510:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 509;
                  break;
               case 511:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 510;
                  break;
               case 512:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 511;
                  break;
               case 513:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 512;
                  break;
               case 515:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 34)
                     kind = 34;
                  jjCheckNAddTwoStates(515, 516);
                  break;
               case 516:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 34)
                     kind = 34;
                  jjCheckNAdd(516);
                  break;
               case 517:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 514;
                  break;
               case 519:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 518;
                  break;
               case 520:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 519;
                  break;
               case 521:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 520;
                  break;
               case 522:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 521;
                  break;
               case 523:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 522;
                  break;
               case 524:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 523;
                  break;
               case 525:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 524;
                  break;
               case 526:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 525;
                  break;
               case 528:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 35)
                     kind = 35;
                  jjCheckNAddTwoStates(528, 529);
                  break;
               case 529:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 35)
                     kind = 35;
                  jjCheckNAdd(529);
                  break;
               case 530:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 527;
                  break;
               case 532:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 531;
                  break;
               case 533:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 532;
                  break;
               case 534:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 533;
                  break;
               case 535:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 534;
                  break;
               case 536:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 535;
                  break;
               case 537:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 536;
                  break;
               case 538:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 537;
                  break;
               case 539:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 538;
                  break;
               case 541:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 36)
                     kind = 36;
                  jjCheckNAddTwoStates(541, 542);
                  break;
               case 542:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 36)
                     kind = 36;
                  jjCheckNAdd(542);
                  break;
               case 543:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 540;
                  break;
               case 545:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 544;
                  break;
               case 546:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 545;
                  break;
               case 547:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 546;
                  break;
               case 548:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 547;
                  break;
               case 549:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 548;
                  break;
               case 550:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 549;
                  break;
               case 551:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 550;
                  break;
               case 552:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 551;
                  break;
               case 554:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 37)
                     kind = 37;
                  jjCheckNAddTwoStates(554, 555);
                  break;
               case 555:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 37)
                     kind = 37;
                  jjCheckNAdd(555);
                  break;
               case 556:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 553;
                  break;
               case 558:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 557;
                  break;
               case 559:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 558;
                  break;
               case 560:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 559;
                  break;
               case 561:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 560;
                  break;
               case 562:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 561;
                  break;
               case 563:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 562;
                  break;
               case 564:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 563;
                  break;
               case 565:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 564;
                  break;
               case 567:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAddTwoStates(567, 568);
                  break;
               case 568:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 38)
                     kind = 38;
                  jjCheckNAdd(568);
                  break;
               case 569:
                  if (curChar == 100)
                     jjstateSet[jjnewStateCnt++] = 566;
                  break;
               case 571:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 570;
                  break;
               case 572:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 571;
                  break;
               case 573:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 572;
                  break;
               case 574:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 573;
                  break;
               case 575:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 574;
                  break;
               case 576:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 575;
                  break;
               case 577:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 576;
                  break;
               case 578:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 577;
                  break;
               case 580:
                  if (curChar == 112)
                     jjCheckNAdd(581);
                  break;
               case 582:
                  if (curChar == 65)
                     jjCheckNAdd(583);
                  break;
               case 584:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjCheckNAddTwoStates(584, 585);
                  break;
               case 585:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 50)
                     kind = 50;
                  jjCheckNAdd(585);
                  break;
               case 586:
                  if (curChar == 67)
                     jjCheckNAdd(582);
                  break;
               case 587:
                  if (curChar == 66)
                     jjCheckNAdd(582);
                  break;
               case 588:
                  if (curChar == 78)
                     jjCheckNAdd(583);
                  break;
               case 589:
                  if (curChar == 73)
                     jjstateSet[jjnewStateCnt++] = 588;
                  break;
               case 590:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 589;
                  break;
               case 591:
                  if (curChar == 68)
                     jjstateSet[jjnewStateCnt++] = 590;
                  break;
               case 592:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 591;
                  break;
               case 593:
                  if (curChar == 84)
                     jjCheckNAdd(583);
                  break;
               case 594:
                  if (curChar == 71)
                     jjstateSet[jjnewStateCnt++] = 593;
                  break;
               case 595:
                  if (curChar == 78)
                     jjstateSet[jjnewStateCnt++] = 594;
                  break;
               case 596:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 595;
                  break;
               case 597:
                  if (curChar == 76)
                     jjCheckNAdd(583);
                  break;
               case 598:
                  if (curChar == 82)
                     jjstateSet[jjnewStateCnt++] = 597;
                  break;
               case 599:
                  if (curChar == 84)
                     jjstateSet[jjnewStateCnt++] = 598;
                  break;
               case 600:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 599;
                  break;
               case 601:
                  if (curChar == 80)
                     jjstateSet[jjnewStateCnt++] = 600;
                  break;
               case 602:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 580;
                  break;
               case 603:
                  if (curChar == 116)
                     jjCheckNAdd(581);
                  break;
               case 604:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 579;
                  break;
               case 605:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 604;
                  break;
               case 606:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 605;
                  break;
               case 608:
                  if (curChar == 109)
                     jjCheckNAdd(609);
                  break;
               case 610:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjCheckNAddTwoStates(610, 611);
                  break;
               case 611:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 55)
                     kind = 55;
                  jjCheckNAdd(611);
                  break;
               case 612:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 608;
                  break;
               case 613:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 612;
                  break;
               case 614:
                  if (curChar == 103)
                     jjCheckNAdd(609);
                  break;
               case 615:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 614;
                  break;
               case 616:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 615;
                  break;
               case 617:
                  if (curChar == 109)
                     jjAddStates(338, 339);
                  break;
               case 618:
                  if (curChar == 110)
                     jjCheckNAdd(609);
                  break;
               case 619:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 618;
                  break;
               case 620:
                  if (curChar == 120)
                     jjCheckNAdd(609);
                  break;
               case 621:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 620;
                  break;
               case 622:
                  if (curChar == 110)
                     jjstateSet[jjnewStateCnt++] = 607;
                  break;
               case 624:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 623;
                  break;
               case 625:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 624;
                  break;
               case 626:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 625;
                  break;
               case 627:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 626;
                  break;
               case 628:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 627;
                  break;
               case 629:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 628;
                  break;
               case 630:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 629;
                  break;
               case 631:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 630;
                  break;
               case 633:
                  if ((0x10100000000000L & l) != 0L)
                     jjstateSet[jjnewStateCnt++] = 646;
                  break;
               case 635:
                  if (curChar == 115)
                     jjCheckNAdd(636);
                  break;
               case 637:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 57)
                     kind = 57;
                  jjCheckNAddTwoStates(637, 638);
                  break;
               case 638:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 57)
                     kind = 57;
                  jjCheckNAdd(638);
                  break;
               case 639:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 635;
                  break;
               case 640:
                  if (curChar == 111)
                     jjstateSet[jjnewStateCnt++] = 639;
                  break;
               case 641:
                  if (curChar == 112)
                     jjCheckNAdd(636);
                  break;
               case 642:
                  if (curChar == 119)
                     jjstateSet[jjnewStateCnt++] = 641;
                  break;
               case 643:
                  if (curChar == 109)
                     jjstateSet[jjnewStateCnt++] = 634;
                  break;
               case 644:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 643;
                  break;
               case 645:
                  if (curChar == 115)
                     jjstateSet[jjnewStateCnt++] = 644;
                  break;
               case 647:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 632;
                  break;
               case 648:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 647;
                  break;
               case 649:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 648;
                  break;
               case 650:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 649;
                  break;
               case 651:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 650;
                  break;
               case 652:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 651;
                  break;
               case 653:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 652;
                  break;
               case 654:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 653;
                  break;
               case 655:
                  if (curChar == 103 && kind > 59)
                     kind = 59;
                  break;
               case 656:
                  if (curChar == 118)
                     jjstateSet[jjnewStateCnt++] = 655;
                  break;
               case 658:
                  if (curChar == 65)
                     jjCheckNAdd(659);
                  break;
               case 660:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddStates(340, 342);
                  break;
               case 661:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddTwoStates(661, 662);
                  break;
               case 663:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddStates(343, 345);
                  break;
               case 664:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 63)
                     kind = 63;
                  jjCheckNAddTwoStates(662, 664);
                  break;
               case 665:
                  if (curChar == 67)
                     jjCheckNAdd(658);
                  break;
               case 666:
                  if (curChar == 66)
                     jjCheckNAdd(658);
                  break;
               case 667:
                  if (curChar == 78)
                     jjCheckNAdd(659);
                  break;
               case 668:
                  if (curChar == 73)
                     jjstateSet[jjnewStateCnt++] = 667;
                  break;
               case 669:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 668;
                  break;
               case 670:
                  if (curChar == 68)
                     jjstateSet[jjnewStateCnt++] = 669;
                  break;
               case 671:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 670;
                  break;
               case 672:
                  if (curChar == 84)
                     jjCheckNAdd(659);
                  break;
               case 673:
                  if (curChar == 71)
                     jjstateSet[jjnewStateCnt++] = 672;
                  break;
               case 674:
                  if (curChar == 78)
                     jjstateSet[jjnewStateCnt++] = 673;
                  break;
               case 675:
                  if (curChar == 77)
                     jjstateSet[jjnewStateCnt++] = 674;
                  break;
               case 676:
                  if (curChar == 76)
                     jjCheckNAdd(659);
                  break;
               case 677:
                  if (curChar == 82)
                     jjstateSet[jjnewStateCnt++] = 676;
                  break;
               case 678:
                  if (curChar == 84)
                     jjstateSet[jjnewStateCnt++] = 677;
                  break;
               case 679:
                  if (curChar == 65)
                     jjstateSet[jjnewStateCnt++] = 678;
                  break;
               case 680:
                  if (curChar == 80)
                     jjstateSet[jjnewStateCnt++] = 679;
                  break;
               case 681:
                  if (curChar == 97)
                     jjstateSet[jjnewStateCnt++] = 657;
                  break;
               case 682:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 681;
                  break;
               case 683:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 682;
                  break;
               case 685:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(346, 348);
                  break;
               case 686:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(686, 687);
                  break;
               case 688:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddStates(349, 351);
                  break;
               case 689:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(689, 690);
                  break;
               case 691:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddStates(352, 354);
                  break;
               case 692:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(690, 692);
                  break;
               case 693:
                  if (curChar == 108)
                     jjstateSet[jjnewStateCnt++] = 684;
                  break;
               case 695:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 694;
                  break;
               case 696:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 695;
                  break;
               case 697:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 696;
                  break;
               case 698:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 697;
                  break;
               case 699:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 698;
                  break;
               case 700:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 699;
                  break;
               case 701:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 700;
                  break;
               case 702:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 701;
                  break;
               case 704:
                  if ((0x7fffffe07fffffeL & l) != 0L)
                     jjCheckNAddStates(355, 357);
                  break;
               case 705:
                  if ((0x7fffffe87fffffeL & l) != 0L)
                     jjCheckNAddTwoStates(705, 706);
                  break;
               case 711:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddStates(358, 360);
                  break;
               case 712:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(712, 713);
                  break;
               case 714:
                  if ((0x7fffffe07fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddStates(361, 363);
                  break;
               case 715:
                  if ((0x7fffffe87fffffeL & l) == 0L)
                     break;
                  if (kind > 64)
                     kind = 64;
                  jjCheckNAddTwoStates(713, 715);
                  break;
               case 718:
                  if ((0x2000000020L & l) != 0L)
                     jjAddStates(364, 366);
                  break;
               case 739:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 703;
                  break;
               case 741:
                  if (curChar == 101)
                     jjstateSet[jjnewStateCnt++] = 740;
                  break;
               case 742:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 741;
                  break;
               case 743:
                  if (curChar == 117)
                     jjstateSet[jjnewStateCnt++] = 742;
                  break;
               case 744:
                  if (curChar == 98)
                     jjstateSet[jjnewStateCnt++] = 743;
                  break;
               case 745:
                  if (curChar == 105)
                     jjstateSet[jjnewStateCnt++] = 744;
                  break;
               case 746:
                  if (curChar == 114)
                     jjstateSet[jjnewStateCnt++] = 745;
                  break;
               case 747:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 746;
                  break;
               case 748:
                  if (curChar == 116)
                     jjstateSet[jjnewStateCnt++] = 747;
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      else
      {
         int i2 = (curChar & 0xff) >> 6;
         long l2 = 1L << (curChar & 077);
         do
         {
            switch(jjstateSet[--i])
            {
               case 75:
               case 77:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddStates(14, 16);
                  break;
               case 111:
                  if ((jjbitVec0[i2] & l2) == 0L)
                     break;
                  if (kind > 5)
                     kind = 5;
                  jjstateSet[jjnewStateCnt++] = 111;
                  break;
               case 113:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddTwoStates(113, 114);
                  break;
               case 115:
               case 116:
                  if ((jjbitVec0[i2] & l2) != 0L)
                     jjCheckNAddTwoStates(116, 114);
                  break;
               default : break;
            }
         } while(i != startsAt);
      }
      if (kind != 0x7fffffff)
      {
         jjmatchedKind = kind;
         jjmatchedPos = curPos;
         kind = 0x7fffffff;
      }
      ++curPos;
      if ((i = jjnewStateCnt) == (startsAt = 759 - (jjnewStateCnt = startsAt)))
         break;
      try { curChar = input_stream.readChar(); }
      catch(java.io.IOException e) { break; }
   }
   if (jjmatchedPos > strPos)
      return curPos;

   int toRet = Math.max(curPos, seenUpto);

   if (curPos < toRet)
      for (i = toRet - Math.min(curPos, seenUpto); i-- > 0; )
         try { curChar = input_stream.readChar(); }
         catch(java.io.IOException e) { throw new Error("Internal Error : Please send a bug report."); }

   if (jjmatchedPos < strPos)
   {
      jjmatchedKind = strKind;
      jjmatchedPos = strPos;
   }
   else if (jjmatchedPos == strPos && jjmatchedKind > strKind)
      jjmatchedKind = strKind;

   return toRet;
}
static final int[] jjnextStates = {
   104, 105, 106, 99, 100, 101, 89, 750, 85, 87, 98, 103, 110, 112, 75, 76, 
   78, 37, 41, 46, 53, 56, 91, 93, 94, 95, 96, 99, 100, 101, 89, 100, 
   101, 89, 104, 105, 106, 114, 115, 117, 131, 146, 156, 171, 181, 196, 206, 221, 
   231, 246, 256, 271, 285, 287, 301, 302, 309, 312, 313, 338, 339, 348, 350, 372, 
   402, 403, 373, 381, 392, 397, 382, 383, 375, 385, 387, 388, 389, 390, 375, 389, 
   390, 375, 393, 394, 395, 383, 375, 393, 383, 375, 394, 395, 383, 375, 396, 383, 
   375, 398, 399, 400, 375, 399, 400, 375, 398, 399, 400, 393, 394, 395, 383, 375, 
   413, 414, 417, 418, 444, 445, 448, 449, 450, 480, 481, 451, 459, 470, 475, 460, 
   461, 453, 463, 465, 466, 467, 468, 453, 467, 468, 453, 471, 472, 473, 461, 453, 
   471, 461, 453, 472, 473, 461, 453, 474, 461, 453, 476, 477, 478, 453, 477, 478, 
   453, 476, 477, 478, 471, 472, 473, 461, 453, 602, 603, 586, 587, 592, 596, 601, 
   613, 616, 617, 640, 642, 665, 666, 671, 675, 680, 686, 687, 705, 706, 707, 737, 
   738, 708, 716, 727, 732, 717, 718, 710, 720, 722, 723, 724, 725, 710, 724, 725, 
   710, 728, 729, 730, 718, 710, 728, 718, 710, 729, 730, 718, 710, 731, 718, 710, 
   733, 734, 735, 710, 734, 735, 710, 733, 734, 735, 728, 729, 730, 718, 710, 513, 
   526, 539, 552, 565, 578, 606, 631, 654, 656, 683, 702, 748, 128, 153, 178, 203, 
   228, 253, 278, 298, 335, 370, 410, 441, 499, 81, 83, 62, 63, 64, 64, 65, 
   66, 90, 91, 93, 130, 131, 146, 155, 156, 171, 180, 181, 196, 205, 206, 221, 
   230, 231, 246, 255, 256, 271, 300, 301, 302, 315, 317, 337, 338, 339, 376, 377, 
   378, 378, 379, 380, 384, 385, 387, 412, 413, 414, 416, 417, 418, 419, 420, 421, 
   421, 422, 423, 443, 444, 445, 447, 448, 449, 454, 455, 456, 456, 457, 458, 462, 
   463, 465, 619, 621, 660, 661, 662, 662, 663, 664, 685, 686, 687, 688, 689, 690, 
   690, 691, 692, 704, 705, 706, 711, 712, 713, 713, 714, 715, 719, 720, 722, 
};

/** Token literal values. */
public static final String[] jjstrLiteralImages = {
"", null, null, null, null, null, null, null, null, null, "\54", "\74", 
"\74\75", "\76", "\76\75", "\75", "\41\75", "\74\76", "\50", "\51", "\57", "\53", "\55", 
"\143\157\156\164\141\151\156\163", "\156\157\164\143\157\156\164\141\151\156\163", "\145\161\165\141\154\163", 
"\142\145\164\167\145\145\156", null, null, null, null, null, null, null, null, null, null, null, null, null, 
null, null, null, null, null, null, null, null, null, null, null, 
"\157\142\163\145\162\166\141\164\151\157\156", "\167\141\171\160\157\151\156\164", "\167\160\156\72\163\162\143", null, null, 
null, null, null, null, null, "\174", null, null, null, null, null, null, };

/** Lexer state names. */
public static final String[] lexStateNames = {
   "DEFAULT",
};
static final long[] jjtoToken = {
   0xfffe7fff8fffff81L, 0xfL, 
};
static final long[] jjtoSkip = {
   0x7eL, 0x0L, 
};
static final long[] jjtoSpecial = {
   0x60L, 0x0L, 
};
protected SimpleCharStream input_stream;
private final int[] jjrounds = new int[759];
private final int[] jjstateSet = new int[1518];
protected char curChar;
/** Constructor. */
public ParserTokenManager(SimpleCharStream stream){
   if (SimpleCharStream.staticFlag)
      throw new Error("ERROR: Cannot use a static CharStream class with a non-static lexical analyzer.");
   input_stream = stream;
}

/** Constructor. */
public ParserTokenManager(SimpleCharStream stream, int lexState){
   this(stream);
   SwitchTo(lexState);
}

/** Reinitialise parser. */
public void ReInit(SimpleCharStream stream)
{
   jjmatchedPos = jjnewStateCnt = 0;
   curLexState = defaultLexState;
   input_stream = stream;
   ReInitRounds();
}
private void ReInitRounds()
{
   int i;
   jjround = 0x80000001;
   for (i = 759; i-- > 0;)
      jjrounds[i] = 0x80000000;
}

/** Reinitialise parser. */
public void ReInit(SimpleCharStream stream, int lexState)
{
   ReInit(stream);
   SwitchTo(lexState);
}

/** Switch to specified lex state. */
public void SwitchTo(int lexState)
{
   if (lexState >= 1 || lexState < 0)
      throw new TokenMgrError("Error: Ignoring invalid lexical state : " + lexState + ". State unchanged.", TokenMgrError.INVALID_LEXICAL_STATE);
   else
      curLexState = lexState;
}

protected Token jjFillToken()
{
   final Token t;
   final String curTokenImage;
   final int beginLine;
   final int endLine;
   final int beginColumn;
   final int endColumn;
   String im = jjstrLiteralImages[jjmatchedKind];
   curTokenImage = (im == null) ? input_stream.GetImage() : im;
   beginLine = input_stream.getBeginLine();
   beginColumn = input_stream.getBeginColumn();
   endLine = input_stream.getEndLine();
   endColumn = input_stream.getEndColumn();
   t = Token.newToken(jjmatchedKind, curTokenImage);

   t.beginLine = beginLine;
   t.endLine = endLine;
   t.beginColumn = beginColumn;
   t.endColumn = endColumn;

   return t;
}

int curLexState = 0;
int defaultLexState = 0;
int jjnewStateCnt;
int jjround;
int jjmatchedPos;
int jjmatchedKind;

/** Get the next Token. */
public Token getNextToken() 
{
  Token specialToken = null;
  Token matchedToken;
  int curPos = 0;

  EOFLoop :
  for (;;)
  {
   try
   {
      curChar = input_stream.BeginToken();
   }
   catch(java.io.IOException e)
   {
      jjmatchedKind = 0;
      matchedToken = jjFillToken();
      matchedToken.specialToken = specialToken;
      return matchedToken;
   }

   jjmatchedKind = 0x7fffffff;
   jjmatchedPos = 0;
   curPos = jjMoveStringLiteralDfa0_0();
   if (jjmatchedKind != 0x7fffffff)
   {
      if (jjmatchedPos + 1 < curPos)
         input_stream.backup(curPos - jjmatchedPos - 1);
      if ((jjtoToken[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L)
      {
         matchedToken = jjFillToken();
         matchedToken.specialToken = specialToken;
         return matchedToken;
      }
      else
      {
         if ((jjtoSpecial[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L)
         {
            matchedToken = jjFillToken();
            if (specialToken == null)
               specialToken = matchedToken;
            else
            {
               matchedToken.specialToken = specialToken;
               specialToken = (specialToken.next = matchedToken);
            }
         }
         continue EOFLoop;
      }
   }
   int error_line = input_stream.getEndLine();
   int error_column = input_stream.getEndColumn();
   String error_after = null;
   boolean EOFSeen = false;
   try { input_stream.readChar(); input_stream.backup(1); }
   catch (java.io.IOException e1) {
      EOFSeen = true;
      error_after = curPos <= 1 ? "" : input_stream.GetImage();
      if (curChar == '\n' || curChar == '\r') {
         error_line++;
         error_column = 0;
      }
      else
         error_column++;
   }
   if (!EOFSeen) {
      input_stream.backup(1);
      error_after = curPos <= 1 ? "" : input_stream.GetImage();
   }
   throw new TokenMgrError(EOFSeen, curLexState, error_line, error_column, error_after, curChar, TokenMgrError.LEXICAL_ERROR);
  }
}

private void jjCheckNAdd(int state)
{
   if (jjrounds[state] != jjround)
   {
      jjstateSet[jjnewStateCnt++] = state;
      jjrounds[state] = jjround;
   }
}
private void jjAddStates(int start, int end)
{
   do {
      jjstateSet[jjnewStateCnt++] = jjnextStates[start];
   } while (start++ != end);
}
private void jjCheckNAddTwoStates(int state1, int state2)
{
   jjCheckNAdd(state1);
   jjCheckNAdd(state2);
}

private void jjCheckNAddStates(int start, int end)
{
   do {
      jjCheckNAdd(jjnextStates[start]);
   } while (start++ != end);
}

}
