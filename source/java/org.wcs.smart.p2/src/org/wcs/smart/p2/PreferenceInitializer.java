...
public class PreferenceInitializer extends AbstractPreferenceInitializer {
	...
	public static void migratePreferences() {
		Preferences pref = new ProfileScope(getDefaultAgentLocation(), IProfileRegistry.SELF).getNode(Activator.PLUGIN_ID);
		try {
	...
	private static IAgentLocation getDefaultAgentLocation() {
		ServiceReference<?> reference = Activator.getContext().getServiceReference(IAgentLocation.SERVICE_NAME);
		if (reference == null)
			return null;
		IAgentLocation result = (IAgentLocation) Activator.getContext().getService(reference);
