...
window.onload = function(){
	...
	document.querySelector("#editdesktopuserform").onsubmit = editDesktopUser;
...
function createDesktopUserTable(){
	...
	var parent = document.querySelector("#deskptopuserstable");
 	...
 	for (var i = 0; i < users.length; i ++){
 		var row = tableCreateRow(parent, 
 				[users[i][0], users[i][1], null, null], 
 				"desktopuserrow " + (i % 2 == 0 ? "smart-table-rowon" : "smart-table-rowoff"));
 		row.dataset.username = users[i][0];
 		row.dataset.cauuid = users[i][2];
 		row.onclick = showDesktopUserInfo;
 	
 		var editicon = document.createElement("a");
 		editicon.className="update-icon";
 		editicon.title="edit user";
 		editicon.dataset.username = users[i][0];
 		editicon.dataset.caUuid = users[i][2];
 		...
 		deleteicon.className="delete-icon";
 		deleteicon.title="deactivate user";
 		deleteicon.dataset.username = users[i][0];
 		deleteicon.dataset.caUuid = users[i][2];
...
function setDesktopUserDetails(){
	...
	var html = "<div style='margin-top: 5px'><span class='label-header'>" + i18n("users.usernamelabel") + "</span><span>" + user[0][0] + "</span></div>";
	html += "<br/><p>" + i18n("users.calistlabel") + "</p>";
	for (var i = 0; i < user.length; i ++){
		html += "<p>" + user[i][1] + " (" + user[i][2] + ")</p>";
	}
	ele.innerHTML = html;
...
function getUserEditDetails(){
	...
	var caUuid = this.dataset.caUuid;
 	...
 	oReq.open("Get", DESKTOP_USER_URL + "ca/" + username + "?cauuid=" + caUuid, true);
 	oReq.send();
 	return false;
} 
...
function showEditDesktopUserDialog(){
	var user = JSON.parse(this.responseText);
	document.querySelector("input[name=edit_dpassword1]").placeholder = "Leave blank unless you want to change password";
	document.querySelector("input[name=edit_dpassword1]").value = "";
 	...
 	document.querySelector("select[name=edit_dca]").value = user.caUuid;
 	...
 	document.querySelector("select[name=edit_userLevel]").value = user.userLevelKey;//this won't work for multi-levels//TODO fix it 	
...
function createNewDesktopUser(){
	var pass1 = document.querySelector("input[name=dpassword1]").value;
	var pass2 = document.querySelector("input[name=dpassword2]").value;
	var user = document.querySelector("input[name=dusername]").value;
	var caUuid= document.querySelector("select[name=dca]").value;
	
	var familyName= document.querySelector("input[name=familyName]").value;
	var givenName = document.querySelector("input[name=givenName]").value;
	var gender = document.querySelector("select[name=gender]").value;
	
	var id = document.querySelector("input[name=id]").value;

	var userLevel = document.querySelector("select[name=userLevel]").value;
	...
	oReq.open("POST", DESKTOP_USER_URL + encodeURIComponent(user), true);
	oReq.setRequestHeader("Content-type", "application/json");
	oReq.send(JSON.stringify(jsonData));
	return false;
}
