				...
				for (ISurveyQueryResultItem it : result) {
					if (muuid.equals(it.getMissionUuid())) {
						if (rs.getObject(3) != null) {
							...
							it.addMissionPropertyValue(key,((MissionAttributeListItem) session.get(MissionAttributeListItem.class,(UUID)rs.getObject(5))).getName());
				...
				for (ISamplingUnitResultItem it : result) {
					if (muuid.equals(it.getSamplingUnitUuid())) {
						if (rs.getObject(3) != null) {
							...
							String value = ((SamplingUnitAttributeListItem) session.get(SamplingUnitAttributeListItem.class,(UUID)rs.getObject(5))).getName();
