...
public class EntityTypeXmlConverter {
	...
	public static org.wcs.smart.entity.xml.model.EntityType toXml(EntityType entityType, IProgressMonitor monitor){
		monitor.beginTask(MessageFormat.format("Exporting Entity Type {0}", new Object[]{entityType.getName()}), entityType.getAttributes().size() + 1);
		...
		for (EntityAttribute et : entityType.getAttributes()){
			monitor.subTask(MessageFormat.format("Exporting attribute {0}", new Object[]{et.getName()}));
