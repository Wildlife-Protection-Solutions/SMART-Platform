...
public class EntityTypeToXmlConverter {
	...
	public static EntityType fromXml(org.wcs.smart.entity.xml.model.EntityType xml, Session session) throws ParseException{
		...
		importNames(xml.getNames(), entityType, session);
		...
		if (dmAttribute != null){
			throw new ParseException(
					MessageFormat.format("The attribute ''{0}'' already exists in the data model.  The entity type cannot be imported until this attribute is removed from the data model or the key is modified.", new Object[]{xml.getDmAttribute().getKey()}), 0); 
		}
		...
		for (org.wcs.smart.entity.xml.model.EntityAttribute xmlAtt : xml.getAttributes()){
			...
			importNames(xmlAtt.getAliases(), ea, session);
			ea.setOrder(order++);
	...
	public static Attribute createAttribute(DataModelAttribute xml, Session session) throws ParseException{
		...
		importNames(xml.getNames(),a, session);
		...
		if (xml.getValues() != null){
			...
			for (int i = 0; i < items.size(); i ++){
				...
				importNames(item.getNames(), newItem, session);
		...
		if (newtype == null){
			throw new ParseException(MessageFormat.format("Attribute type {0} not supported.", new Object[]{xml.getType()}), 0);
		}
		a.setType(newtype);
	...
	private static Aggregation lookUpAggregation(String name) throws ParseException{
		...
		throw new ParseException(MessageFormat.format("Aggregation {0} not found.", new Object[]{name}), 0);
	}
	
	private  static Attribute findAttribute(String keyId, Session s){
		List<?> attributes = s.createCriteria(Attribute.class).add(Restrictions.eq("keyId", keyId)).add(Restrictions.eq("conservationArea", SmartDB.getCurrentConservationArea())).list();
	...
	private static AttributeTreeNode processAttributeTreeNode(AttributeTreeNode parent, 
		...
		importNames(xmlNode.getNames(), newAttributeTreeNode, session);
	 ...
	 * @throws Exception
	 */
	public static void importNames(List<LabelType> names, NamedItem toUpdate, Session session) {
